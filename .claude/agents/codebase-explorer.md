...
src/shared/          — Shared utilities (no CSS dependency)
```

Key patterns:
- **Multiple Root Layouts**: `(admin)` and `(public)` have separate `layout.tsx` with `<html>` tags
- **Section-based pages**: `(public)/[slug]/page.tsx` renders DB-driven sections
...
- Data flow paths (DB -> Server Action -> Cache -> Component)
