...
- **フォーム** (Phase 1 Task 4-8 全完了の conform canonical): 新規 admin form は React 19 `useActionState` + conform `useForm` (`@conform-to/react`) + `parseWithZod` (`@conform-to/zod/v4`) + `executeConformMutation` SSoT helper 経由で `executeAdminMutationResult` を呼ぶ。Server Action は `(prev, formData) => SubmissionResult` signature、id 必要時は `bind` で部分適用。`useFormAction` (RHF) は legacy で新規利用禁止 (Phase 1 残存 = inline editor side-panel / auto-section-form のみ、別 phase で `react-hook-form` / `@hookform/resolvers` を `package.json` から削除予定)、既存 RHF 残存 form の編集時は同じ commit 内で conform に置換する。`useFormStatus` で pending を取らない（`SubmitButton` + `useActionState` 第 3 戻り値 `isPending` を prop で渡す）。動的配列は ① `form.insert/remove/reorder` + `getFieldList()` + `getFieldset()` (LocationForm canonical) または ② `useState<{key, ...}[]>` + 安定 key + hidden input append + schema preprocess (SpaceEditForm canonical、MediaPicker / IconPickerField 連携時) のいずれかを採用。参照実装: `CustomerForm` / `CustomerEditForm` / `CouponForm` / `PageSeoForm` / `UserForm` / `InviteForm` / `space-categories/_components/CategoryForm.tsx` (Dialog 内 Variant A, PR #64) / `posts/taxonomy/_components/TaxonomyEditor.tsx` (AdminDetailLayout 編集ページ dual-impl, PR #68) / `LocationForm` (PR #96、form.reorder + dnd-kit) / `SpaceEditForm` (PR #98、5 tab monolithic + useState array transit)
