---
name: add-cron-job
description: 定期実行ジョブ (cron job) を新規追加するときに使う。src/app/api/cron/ への Route Handler 追加、Cloud Scheduler OIDC Bearer token 認可 (authorizeCronRequest・fail-closed)、feature module OFF 時の早期 return、cache invalidation (revalidateTag / invalidateSiteWideCacheFromRouteHandler)、terraform/cloud_scheduler.tf への scheduler 登録、unit test までの一連の手順とチェックリスト。
---
...
- `unstable_rethrow` — `next/navigation`。catch 節の先頭で呼ぶ (全 cron route 共通)
...
- **architecture gate への登録は不要**:
  `__tests__/unit/architecture/cron-oidc-clean-break.test.ts` は
  `src/app/api/cron` を自動走査するので、route を置けば
  「`await connection()` が `await authorizeCronRequest` より前」が
  ファイル単位で自動検査される
...
### 6. Cloud Scheduler へ登録 (Terraform)

`terraform/cloud_scheduler.tf` の `locals.cron_jobs` に 1 entry 追加する:
...
```hcl
{
  ...
  description = "<運用者向け説明(英語)>"
},
```

- cron 式は `time_zone = "Asia/Tokyo"` で解釈される。OIDC token 発行・
  `attempt_deadline = "300s"`・`retry_count = 3` は resource 側の共通設定
- 登録は GitHub Actions の terraform workflow が plan → merge で apply する。
  手動 `gcloud` は使わない
- **deploy 成功後に follow-up PR で `locals.imported_cron_jobs` にも追加する**
  (fresh state からの apply で 409 にならないための段階 B。同ファイル冒頭の説明を参照)
- route ディレクトリと `path` の完全一致は
  `__tests__/unit/architecture/cron-scheduler-path-sync.test.ts` が強制する
  ので、片方だけ足すと赤くなる
...
- [ ] unit test 追加 (gate への登録は不要 — 自動走査)
- [ ] `terraform/cloud_scheduler.tf` の `locals.cron_jobs` に entry 追加
      (deploy 成功後に `imported_cron_jobs` への follow-up PR)
