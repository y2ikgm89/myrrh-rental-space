...
argument-hint: <SectionName> [hero|content|cta|stacking|gallery|zoom|sequence|split]
...
```bash
python3 .claude/skills/ui-ux-pro-max/scripts/search.py "<業種キーワード>" --domain product
python3 .claude/skills/ui-ux-pro-max/scripts/search.py "motion parallax scroll" --domain style
python3 .claude/skills/ui-ux-pro-max/scripts/search.py "scroll storytelling hero" --domain landing
python3 .claude/skills/ui-ux-pro-max/scripts/search.py "<ムードキーワード>" --domain color
python3 .claude/skills/ui-ux-pro-max/scripts/search.py "animation scroll performance" --domain ux
...
- `.claude/rules/visual-effects-patterns.md` — エフェクトレベルアーキテクチャ
- `.claude/rules/threejs-patterns.md` — Three.js / R3F（拡張時）
- `.claude/rules/pixijs-patterns.md` — PixiJS v8（拡張時）
...
**パターン選択**: → `.claude/rules/gsap-patterns.md` §reduced-motion 対応
