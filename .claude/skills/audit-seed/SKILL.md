...
when_to_use: 新規 Prisma モデル追加後、enum 値追加後、または定期メンテ時。add-prisma-enum skill 実行後の確認にも使う。
paths:
  - prisma/seed.ts
...
- `.claude/rules/implementation-patterns.md` — 「Seed 関数は upsert で idempotent 化 + seedAll / seedDemo 両方に登録」
