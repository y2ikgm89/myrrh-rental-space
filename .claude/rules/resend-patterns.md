...
- **Stripe API version `2026-03-25.dahlia`** — stripe SDK v21 の `LatestApiVersion` に合わせる。SDK アップグレード時は `bun run type-check` の型エラーで新バージョン文字列が判明 → `stripe.ts` の `apiVersion` を更新
