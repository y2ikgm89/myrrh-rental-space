...
- **`PrismaPg` は explicit `Pool` インスタンスを渡す** — `new PrismaPg({ connectionString, max, ... })` のように config 渡しだと `connect()` 毎に新 Pool を作る（adapter-pg 7.7 の内部実装、`node_modules/@prisma/adapter-pg/dist/index.mjs:752`）。`new Pool(...)` を渡すと `externalPool` 経路で 1 Pool が再利用される。`src/shared/db/prisma.ts` が dev global singleton で保持
...
- **`pg` は `overrides` で単一バージョンに固定（dual-instance 防止）**: `pg` を `@prisma/adapter-pg` の dep 範囲（`^8.16.3`）内でも top-level だけ更新すると、`@prisma/adapter-pg` が `node_modules/@prisma/adapter-pg/node_modules/pg` に別バージョンの `pg` を nested 解決して `pg` が 2 インスタンスに分裂する。`src/shared/db/prisma.ts` は top-level `pg` の `Pool` を生成して `PrismaPg` に渡すため、adapter が内部の別 `pg` で処理すると `Pool`/`Client` 型不整合で全クエリが `ERR_INVALID_ARG_TYPE` を throw し、ビルド prerender が `USE_CACHE_TIMEOUT` で失敗する。`package.json` の `overrides: { "pg": "8.20.0" }` で全 `pg` を単一バージョンに強制する。`pg` を上げる際は override も同時更新し、`@prisma/adapter-pg` がバンドルする `pg` と齟齬が出ないことを確認する
