...
- **`useFormStatus` は conform `useActionState` + native `<form action={action}>` で正しく動作する** — Phase 1 Task 4-6 で conform 1.19 + `useActionState(serverAction, undefined)` + `<form action={action}>` パターンに統一済み、`useFormStatus` 内 `pending` は SubmitButton 子孫で取得可能。残存 RHF 利用箇所（`useFormAction` + `useTransition` / `useActionState` + RHF ハイブリッド）は **Phase 1 Task 7-8 で削除予定**、当該 form 編集時のみ `SubmitButton` に `isPending` prop で待機状態を渡す legacy パターンを維持する。新規 form は `<form action={action}>` + `useFormStatus` 子孫が canonical
