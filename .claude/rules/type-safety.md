...
- **`Object.fromEntries(arr.map(x => [k, v]))` は callback 戻り値をタプル注釈しないと `any` に落ちる** — `.map(x => [k, v])` の戻り値は TS 仕様で **タプルではなく配列** `(K | V)[]` に推論される。配列は `Object.fromEntries` の第 1 オーバーロード `Iterable<readonly [PropertyKey, T]>` に代入不可のため第 2 オーバーロード `Iterable<readonly any[]> => any` にマッチし、**構築全体が `any`** になる。`const x: Record<string, V> = Object.fromEntries(...)` のように変数注釈で受けても「各値が実際に `V` か」の検証が効かず、`architecture-boundaries.test.ts` の `as` gate もすり抜ける silent な型安全ホール（explicit `as` より検出が困難で悪質）。**必ず callback の戻り値をタプルに明示注釈する**: `arr.map((x): [string, V] => [k, v])`。これで generic オーバーロード（`=> { [k: string]: V }`）にマッチし完全型付けになる。検出 grep: `grep -rnE 'Object\.fromEntries\(' src/` → 各 hit の `.map(` callback にタプル注釈があるか確認（`Object.entries(...).filter(...)` 由来の入力は既にタプルなので対象外）。2026-06-01 に PAGE_TEMPLATES / section-metadata ×4 / FeatureModulesForm / ReservationEditForm の 7 箇所を一括修正
