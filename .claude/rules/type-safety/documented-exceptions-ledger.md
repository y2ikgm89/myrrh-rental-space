...
| `src/app/(admin)/admin/(dashboard)/_shared/components/editor/inline/side-panel/LayoutFields.tsx`        |         155-160         | `as unknown as FieldMetadata<string \| null \| undefined>` / `as unknown as FieldMetadata<number \| null \| undefined>` | §5 conform generic invariance | `FieldMetadata<T>` invariant、`fields["contentWidth"] / ["contentWidthCustom"]` の値型を Pure Component 境界 cast (Phase 3-B Sub-Chunk 2c で RHF `Path<T>` cast から rename、`useInputControl` の控えめ型に合わせる)           |
