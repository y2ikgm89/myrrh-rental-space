...
- **GitHub Actions / npm package / GitHub API spec のバージョン literal を training data から推測しない** — `peter-evans/create-pull-request@v6` / `actions/checkout@v4` 等の literal は training data cutoff で stale 化する。**新規実装 / upgrade 時の SSoT**: `curl -s "https://api.github.com/repos/<o>/<r>/releases/latest" | python3 -c "import json,sys; print(json.load(sys.stdin)['tag_name'])"` で最新 major を取得 + context7 / WebFetch で breaking changes 確認。実例: 2026-05-13 ci.yml で v4-v9 系を使い続け、ユーザ指摘後に全 8 actions を Node 24 対応 major (`@v6/@v7/@v5/@v8/@v10`) に一括 upgrade。`.claude/rules/ops/ci-workflow.md` §7 の監査表を「全 workflow ファイル × 全 7 種類 actions 横断」に拡張済 — 同種ミスの早期検出用 grep を活用
- **「ぬけ漏れ確認」要求時は systemic grep で同種パターンを全 file 横断する** — 単一 file (`ci.yml` 等) のみ focus せず、`grep -rnE '<pattern>' .github/workflows/ .claude/rules/ AGENTS.md CLAUDE.md docs/` で全 workflow file / rule docs / docs 横断検索。`renovate.json5` / `dependabot.yml` / `.github/CODEOWNERS` 等の周辺 config も同時確認。実例: 2026-05-13 セッション「ぬけ漏れ確認」で `ci.yml` のみ focus した結果、`actionlint.yml` / `dependency-review.yml` / `labeler.yml` / `stale.yml` の 4 workflow の旧 actions 残存を見落とし → ユーザ指摘で発覚 → systemic grep に統一。`ci-workflow.md` §7 の監査 grep を「全 7 種類 actions × 全 workflow file 横断」に拡張済
...
- **`.claude/rules/**/_.md`の「N resource 統一済」等 SSoT 宣言は codification 先行 / implementation 追従のことがある** — rule 宣言を信じる前に`grep -rn "<old-pattern>" src/`で実態確認必須。codification commit と implementation commit が分離し partial state で残るケースの drift。実例: 2026-05-12`code-quality/forbidden-patterns.md`§7 +`frontend/admin-ui-patterns.md`§Gotchas が「9 resource 統一済」宣言 (commit`05692bc8`) を持つが Space (`toggleSpacePublishedCommand`) / Location (`toggleLocationPublish`) は実装側 `toggle_`のまま残存 → 後続 commit`fe7f6a09` で完遂
...
- **Worktree 採否は `.claude/rules/git-migration.md` §Worktree 採否判定 を参照** — Phase 分割 plan / subagent isolation / destructive migration → worktree、1-commit / config-only / 短時間 fix → main 直接編集。判定根拠（worktree path Read で `.claude/rules/**` の 2 セット auto-load で context 倍消費等）と bootstrap 手順は同 rule + `worktree-bootstrap` SKILL に集約済み
...
- **管理画面ログイン URL 生成は `bun scripts/generate-login-url.ts`** — Admin Gate トークン付き URL を出力（30 日有効・ワンタイム）。**dev 環境 (`NODE_ENV === "development"`) では proxy.ts が Gate を bypass + `/admin/login` に SUPER_ADMIN ワンクリックボタン (`DevLoginButton.tsx`) が出るため不要**（2026-05-12 以降）。staging / production の Playwright / 手動テストでのみ必要。seed のデフォルト credential は `superadmin@example.com` / `superadmin123`（`prisma/seed.ts:312-315`）。`admin-test@example.com` 等は seed 外の手動作成で password 不明、avoid
...
- **`bun -e` で Prisma 直接アクセスする canonical pattern** — `@/shared/db/prisma` は `import "server-only"` でブロックされるため `bun -e` から使えない。`PrismaClient` + `PrismaPg` adapter を直接 instantiate する: `import { PrismaClient } from './generated/prisma/client'; import { PrismaPg } from '@prisma/adapter-pg'; import { Pool } from 'pg'; const pool = new Pool({ connectionString: process.env['DATABASE_URL'] }); const prisma = new PrismaClient({ adapter: new PrismaPg(pool) }); /* queries */ await prisma.$disconnect(); await pool.end();`。Prisma 7 では adapter 必須（adapter なしでは `PrismaClientInitializationError`）。参照実装: `scripts/generate-login-url.ts` / `scripts/e2e/ensure-admin-user.ts`（PrismaPg + Pool 直接 instantiate の canonical pattern）。DB drift 確認・section.config inspect・ad-hoc 1 回限り操作で使用
...
- **Playwright MCP の HMR キャッシュ罠** — Edit/Write 直後の `browser_navigate`（または同 URL への再アクセス）はブラウザが古い bundle をキャッシュし、コード変更が画面に反映されないことがある（Turbopack HMR が走っていても navigate 経由では新 bundle を取得しないケース）。**確実な反映には `browser_evaluate("() => window.location.reload()")` で強制リロード必須**。スクショ撮影前に `browser_evaluate` で `document.querySelector(...).className` を取り、実際の DOM 上の class が期待した新クラス（例: `flex-wrap items-center`）になっているか確認すると HMR 状態を1往復で判定できる。`getBoundingClientRect()` + `getComputedStyle()` を `browser_evaluate` 経由で取れば overflow / flex-direction / scrollHeight 等の layout 実態を確認できる（スクショだけでは不可視）。実例: 2026-05-12 セッションで CalendarToolbar の `flex-wrap` 修正後、navigate 経由では 3 段組のまま、reload 後に1段組へ正常切替確認
- **HMR が完全に止まった場合の disk state 独立検証** — `window.location.reload()` でも `browser_evaluate` 取得の className が古いままなら Turbopack HMR が停止している (Windows file watcher の既知 instability)。**SSR HTML を直接 fetch して disk state を独立検証**: `browser_evaluate("async () => fetch('/path?cb='+Date.now(), {cache:'no-store'}).then(r=>r.text()).then(html => html.match(/<pattern>/))")` で server-rendered HTML から新コードが反映されているか確認できる。fresh fetch HTML が新 className を含むのに `browser_evaluate(getComputedStyle)` が古い値を返すなら**client bundle 単独の HMR fail**で、commit + CI で disk state 検証に切替えるのが最速。実例: 2026-05-15 PR #31 で StandardHeroSection 編集後、Playwright MCP の DOM は古いまま、fresh fetch HTML は新 className を確認 → そのまま commit + CI 検証で broad E2E green を確認
- **業界標準を citation して design 判断する前に Playwright MCP で実測必須** — 「Kinfolk / Cereal / Stripe / Apple 等のフッターは XXpx」のような citation を training data から出すと、実測値と乖離するリスクが高い（training cutoff 後の改修 / 自分の記憶曖昧 / そもそも未参照）。canonical: ① `browser_resize` で desktop (1440) / wide (1920) 両方 ② `browser_navigate` ③ `browser_evaluate` で computed style 測定 ④ 実測値を表で提示してから判断。**測定 helper pattern**: `<footer>` 直 padding は 0 のサイトが多いため `topGap = first visible text top - footer rect.top` / `bottomGap = footer rect.bottom - last visible text bottom` を visible filter（`clipPath: inset(50%)` / `visibility: hidden` / 4×4px 未満を除外）と組合せて統一比較する。**業界知見**: 主要サイト (Cereal / Stripe / Notion / Linear) は footer padding を viewport 連動 fluid scaling せず fixed px を採用、fluid は editorial decorative 用途のみ。**日本のレンタルスペース / 予約系 (Spacemarket / Instabase / Airbnb)** は semantic `<footer>` 非使用が多く、競合 footer 比較ができない。実例: 2026-05-13 セッションで `--space-xl` (max 240px) フッター適用が Cereal 100px の 2.4 倍と判明、`--space-md` (max 112px) に縮小
- **Goal hook + AskUserQuestion 時の Stop hook 継続要求** — `/goal` で session-scoped Stop hook を設定済みの状態で `AskUserQuestion` を出すと、ユーザが reject した場合に Stop hook が「実装が始まっていない」と判定して継続要求してくる（goal condition が満たされない限り停止不可）。**Goal の condition 文が「A もしくは B」「A または B」「どちらか」のような両カバー選択肢を含むなら、AskUserQuestion を挟まず即実装に入る**のが正解（どちらの選択肢でも goal が満たされるため確認自体が冗長）。複数案がある場合は最も効果の大きい案を選んで方針を1行表明してから着手。条件が単一の場合のみ AskUserQuestion が適切。実例: 2026-05-12 セッションで「予約カレンダー UI 改善もしくは大幅刷新」goal に対し方向確認したら reject + Stop hook 継続発火
- **architecture-boundaries test の fs traverse は 5s default timeout を超える** — `collectSourceFiles(APP_ROUTE_ROOT)` で全 app route (~880 file) を scan する test (`cacheComponents 有効時は route segment config export を残さない` 等) は `test(name, fn, 30000)` で timeout 延長必須。lefthook pre-push hook を block しないように。実例: 2026-05-13 セッションで 600 commit push 時に 9.5s 計測で pre-push fail → timeout 30s 延長で recovery
- **dev server 競合検出** — 既に port 3000 起動中で `bun run dev` 試行すると `Another next dev server is already running` で exit 1（port 3001 fallback はするが）。事前に `tail -5 <task output>` で「Ready in」or「already running」を確認してから Playwright MCP 起動判断する。`taskkill /PID <n>` は MINGW64 で `/PID` がパス解釈されるため `cmd //c "taskkill /PID <n> /F /T"` 経由で kill。Bash `run_in_background: true` で起動した dev server は別 terminal user 起動済との競合検出が必要
- **`bun -e` で dev customer に予約紐付け** — data 依存 mypage (`/mypage/reservations/[id]` 等) を Playwright access するため `prisma.reservation.update({ where: { id }, data: { customerId: devCustomerId } })` で既存予約を dev customer に紐付け。`createAppPrismaClient` ではなく `PrismaClient + PrismaPg + Pool` 直接 instantiate で `server-only` bypass（`bun-patterns.md` §`bun -e` の canonical pattern 参照）。実例: 2026-05-13 セッションで `/mypage/reservations/[id]` 検証時に dev customer (`abb5000a-...`) に既存予約 1 件 link して access 可能化
