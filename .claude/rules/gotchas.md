...
- **機能削除・大規模リネーム時は `.claude/rules/**/_.md`+`docs/reference/codex-rules/\*\*/_.md`も必ず grep** — コード・テスト・seed を消しても、rule docs の bullet が古い関数名・列名・参照実装ファイルを「必須」として残ったままになり、次セッションで誤情報として自動ロードされる。削除対象のシンボル・ファイル名を`grep -r <name> .claude/rules docs/reference/codex-rules`で 0 件確認すること。policy 系を編集したら`node scripts/verify-policy-docs.mjs` で byte-identical 同期（`policy-docs-sync` CI ブロッカー回避）
