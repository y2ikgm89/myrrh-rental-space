...
bun run test:unit          # 全 unit を scripts/run-tests.ts 経由で per-file 実行
...
per-directory `&&` チェーンでも 1 ディレクトリ内に複数 _.test.ts があれば干渉する（過去 30 run 連続 CI failure の主因、78 件規模の偽陽性 fail）。`scripts/run-tests.ts` で各 _.test.ts を **独立した bun サブプロセス**で順次起動し、干渉を物理的に排除する:

```bash
bun scripts/run-tests.ts __tests__/unit         # 全 *.test.ts を再帰探索 → 個別起動
bun scripts/run-tests.ts __tests__/integration
bun scripts/run-tests.ts __tests__/unit __tests__/integration  # 複数 dir 連結
```
