...
| 定数/変数                                                                          | 場所                            | メモ                                                                                                                                              |
| ---------------------------------------------------------------------------------- | ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CACHE_TAGS` / `getCacheTag` / `CACHE_LIFE`                                        | `@/shared/lib/constants`        | `CACHE_TAGS.SETTINGS` は廃止済 → 個別タグ。`SECTION_STYLES` は Style Library 一覧/詳細タグ（`getCacheTag.sectionStyles.detail(id)` ヘルパー付き） |
...
| `listSectionStyles` / `getSectionStyleById` / `createSectionStyle` / `updateSectionStyle` / `deleteSectionStyle` / `deriveSectionStyle` | `@/shared/domain/section-styles/{queries,commands}` | Style Library CRUD の SSoT。Server Action は `@/app/(admin)/admin/(dashboard)/_shared/actions/section-styles/*` で wrap（ADR 0017）。直接 Prisma `sectionStyle.*` 呼び出し禁止                                                                                                                                                                                         |
