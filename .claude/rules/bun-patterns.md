...
- **`ensure-next-types.ts` の `.next/dev/types/**/_.ts`除去は意図的（削除禁止）** —`.next/dev/types/`は`next dev`（Turbopack）のみが生成する dev 専用型成果物で、`next typegen`（`.next/types/`を生成）では再生成されない。dev server 稼働中に`bun run type-check`/`bun run build`を並行実行すると`.next/dev/types/routes.d.ts`が書き込み競合で破損し（行頭`ppRoutes> {`等の truncation で`TS1005`/`TS1128`）、**dev server 停止後も破損ファイルが残って tsc が読み続けて失敗**する（`bun run build`は`.next/types/`を独自生成するため緑 + CI には`.next/dev/types`自体が無いため緑 → ローカル type-check / lefthook / Stop hook だけが赤になる silent block）。tsconfig の`.next/dev/types/\*\*/_.ts`include は **Next.js 16 が自動管理する公式構成**（[Next.js TypeScript config](https://nextjs.org/docs/app/api-reference/config/typescript)）のため除去すると next dev/build/typegen で再注入される。よって対策は`scripts/ensure-next-types.ts`末尾で dev 専用成果物を`Bun.Glob`+`Bun.file().delete()`で消し、tsc を CI と同じく`.next/types/`のみで決定論検証する（自己修復、2026-06-04 確立）。この cleanup を削除すると dev-server 並行実行後の type-check が再び break する。手動回避が必要な旧来の対処は`python3 -c "import shutil; shutil.rmtree('.next/dev/types', ignore_errors=True)"`
