...
- **`useFormAction` (RHF) は legacy** — 新規利用禁止 (Phase 1 残存は inline editor side-panel / auto-section-form のみ、別 phase 完了後に `react-hook-form` / `@hookform/resolvers` を `package.json` から削除予定)。既存 RHF 残存 form の編集時は同 commit 内で conform に置換する。
