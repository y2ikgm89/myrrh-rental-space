...
- **参照実装** (PR #61-#62 conform canonical): `MaintenanceSection` / `CustomerForm` / `CustomerEditForm` / `CouponForm` / `PageSeoForm` / `UserForm` / `InviteForm`。Dialog 内 conform Variant A (PR #64): `space-categories/_components/CategoryForm.tsx` + `CreateCategoryDialog.tsx` / `CategoryActionCell.tsx`。AdminDetailLayout 編集ページ dual-impl (PR #68): `posts/taxonomy/_components/TaxonomyEditor.tsx`（`CategoryEditorImpl` / `TagEditorImpl` を別 `useActionState` で持つ self-contained edit form、共通 OGP preview は内部 subcomponent、新 Server Action signature `updatePostCategoryAction` / `updatePostTagAction` を `bind` 部分適用、legacy `(input)` 版は dialog form 互換のため並列維持）。中間状態 (RHF + FormData): `submitSpaceFormAction` (`@/admin/actions/space-form-submit`)、`space-form-data-codec.ts`、`SpaceEditForm.tsx`（Task 7 で完全 conform 化予定）。
