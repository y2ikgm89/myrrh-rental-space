...
- **Tabler / lucide / heroicons 等の icon library 名前は `node -e` で getter 存在確認してから登録** — schema / curation list に `IconXxx` を直書きしても package が export していないと type-check で `Module has no exported member 'IconXxx'` の silent build error。`node -e "const i = require('@tabler/icons-react'); console.log(Object.keys(i).filter(k => /pattern/i.test(k)).slice(0,30))"` で実在 export を fuzzy 検索してから登録する。30+ entry のキュレーションリストを書き起こす際は build 前に必ず実行（実例: 2026-05-07 IconPickerDialog 構築で `IconFootprint` が不在 → type-check で発覚 → `IconShoe` に置換）
