...
- **dev / staging / production すべてで `CRON_SECRET` 設定必須** — `proxy.ts` の cron 認証は fail-closed: 未設定時は環境問わず 401 を返す（Vercel Cron / Cloud Scheduler / GitHub Actions の業界標準）。本番は `validateProductionEnv()` で起動時 throw、dev/staging は proxy で拒否。ローカルで cron route をテストする場合は `.env.local` に `CRON_SECRET=<32文字以上>` を設定する
...
- **`z.enum(...).default(X)` の input 型 optional 化は conform で吸収済** — Zod 4 公式挙動で `.default()` 有りの `z.input` 型は optional として推論される。conform `useForm` + `parseWithZod` + `getZodConstraint` (`@conform-to/zod/v4` 専用 subpath) は schema input 型を強制せず FormData → `parseWithZod` で常に target type を出力するため `.default()` を残したまま安全に利用可能 (React Hook Form は完全削除済、`standardSchemaResolver` 経由の input 型 mismatch は再発しない)
...
- **`bun run test:unit` / `test:integration` は per-file isolation runner（`scripts/run-tests.ts`）経由で全 file を順次実行** — 1 file fail でも残り file の結果は集計される（exit 1 で完了報告するが、failed file 一覧が末尾に出る）。`per-directory && チェーン` は 2026-05-13 で全廃済（mock.module 干渉の根本解決のため）。詳細は `.claude/rules/bun-patterns/test-runner.md` §per-file isolation runner / `ops/ci-workflow.md` §5
...
- **CI で `Invalid environment variables` で `next start` 起動失敗 → Playwright webServer タイムアウト** — `@t3-oss/env-nextjs` の `clientEnv` validation で `NEXT_PUBLIC_BASE_URL` 等が未設定。`workflow-global env` に全 NEXT*PUBLIC*\* を渡す。build job だけに env を書くと E2E / Visual / Lighthouse が再 fail。詳細 → `ops/ci-workflow.md` §6
- **`The hosted runner lost communication with the server`** — runner の異常終了。3 原因のいずれか: ① test の無限 hang (Playwright で 1h+ 走り続け / config の timeout 不足) ② OOM ③ network 枯渇。`gh run view <id>` で異常長時間 step を確認 → 該当 job を opt-in label trigger に切替。詳細 → `ops/ci-workflow/debug.md` §"runner lost communication with the server"
- **`The job was not started because recent account payments have failed or your spending limit needs to be increased`** — GitHub Actions billing 起因の job skip。private repo + Free tier (2,000 min/月) 上限到達 or payment method 失敗。`gh run rerun <id> --failed` で部分 retry 可能だが、minute 上限解決には ① public repo 化 ② Pro plan ($4/月) ③ spending limit 引き上げ ④ 次月の reset 待ち。詳細 → `ops/ci-workflow/debug.md` §Billing
