...
- **`z.enum(...).default(X)` + RHF `standardSchemaResolver` は input 型を optional 化 (残存 RHF 利用箇所限定、Phase 1 Task 8 完了、別 phase で残存 RHF 完全削除予定)** — `.default()` 有りで `z.input` 型が optional として推論されるため、RHF の form value 型が `T | undefined` となり Select/Input の `value` prop に undefined が流入。対処: schema から `.default()` を削除し UI の `defaultValues` で補う。**新規 form は conform `useForm` + `parseWithZod` + `getZodConstraint` (`@conform-to/zod/v4` 専用 subpath) が canonical で本 gotcha は不発生** — conform は schema input 型を強制せず FormData → `parseWithZod` で常に target type を出力するため `.default()` を残したまま安全に利用可能
