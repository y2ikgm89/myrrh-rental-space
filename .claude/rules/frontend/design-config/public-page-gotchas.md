...
- **`EventInfoPanel` は Split Hero Card pattern (Lu.ma / Cal.com 業界標準)** — 3 ゾーン構造: ① **Hero block** (`bg-surface px-8 pt-8 pb-8 sm:px-10 sm:pt-10 sm:pb-10`) に Badge + 価格 hero (`font-heading font-normal text-[2.5rem]` active 状態 / `text-3xl opacity-60` inactive 状態 deadline-passed/closed) + 「税込・1 名あたり」 (`text-sm text-muted-foreground`、`/` 区切り禁止 — Peatix 業界標準整合) ② **Detail list** (`bg-background dl px-8 sm:px-10`) で日時 + 場所 + 定員 (capacity は detail list 末尾、hero block には載せない) ③ **CTA block** (`registration.kind === "open"` のみ、`bg-foreground text-background min-h-12 sharp edge`、editorial 規約 `rounded-md` 禁止)。**開催日時 DetailRow は 2 行表示**: `<span flex-col gap-0.5>` で `formatEventDate(start)` + `formatEventTimeRange(start, end)` を両方 `text-sm text-foreground` の同等級で並列 (Lu.ma / Peatix / Airbnb / Google Calendar 業界標準、日付 / 時間は同等の意思決定情報)
- **Badge `variant="default"` は `bg-surface` 上で背景同色に溶ける silent bug** — Badge primitive の `default` variant は `bg-surface text-foreground` で `EventInfoPanel.HeroBlock` (`bg-surface` warm cream) 等の同色背景上で枠が完全に消える。`bg-surface` 上のネガティブ状態 Badge (deadline-passed / closed / archived 等) は `bg-background + border border-border + text-muted-foreground` の outline 枠 (例: `EventInfoPanel.OutlineBadge`) で local 回避。Badge primitive 自体の variant 拡張 (`outline` 追加等) は影響範囲確認 + 全 consumer 用法洗い出しが必要なため別 PR 推奨
