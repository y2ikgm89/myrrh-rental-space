...
- **公開詳細ページは 5 系統全て `ArticleLayout` 統一**（PR #246-#250、2026-05-26）— spaces / events / news / posts / terms 全て `bg-surface py-2 shadow-inner` パンくず帯 + `ArticleHeader` (eyebrow + h1 + hairline divider w-16 + meta + media、Kinfolk hairline pattern) + 本文 + optional `<SiteCTA>`。旧「②固定型 `PageHero variant="compact"` + Section」廃止（`_shared/components/layouts/page-hero.tsx` 削除済）。**カテゴリ別 hero 配置** (`ArticleLayout.heroPosition` prop で分岐): ① 予約サービス (spaces / events) は `"full-width"` (default、Airbnb / Booking.com / Eventbrite 業界標準で gallery を main visual 強調) ② ブログ記事 (posts / news) は `"in-grid"` (WordPress Astra / GeneratePress / Newspaper / Hashnode 業界標準で Search / Recent / Popular widget を hero 同 row sticky で常時可視) ③ Docs / 規約 (terms) は `"in-grid"` (Stripe Terms / GitHub TOS / Notion Terms 業界標準で TOC を hero 同 row sticky で navigation friction 低減)。**hero media は `ArticleHeader` 内で `mx-auto max-w-4xl` (約 896px) 制約** (PR #249、NYT / Medium / Kinfolk 記事 hero thumbnail 標準、container 1280px 全幅 × 16:9 = 720px 縦巨大化を回避)。**events 固有**: `ArticleLayout` の `toc` / `mobileToc` slot に `EventInfoPanel`（右サイド sticky 情報カード / mobile 本文冒頭 inline、Eventbrite / Peatix / Lu.ma 業界標準）を注入し、CTA は `#event-register` アンカー + `scroll-mt-[calc(var(--header-height)+2rem)]` で本文末尾の登録フォームへ誘導。`AddToCalendar` は主導線位置から退避し `<details>` 折りたたみ + 「予約しなくても追加できます」注記で `ArticleFooter` 直前に控えめ配置（業界標準 — 主 CTA を阻害しないリマインダー UX）。**spaces 固有**: `ReservationWidget` を `toc` / `mobileToc` slot で配線（Airbnb pricing widget パターン、`BreadcrumbJsonLd` は `Breadcrumb` 内部出力に一本化して二重出力 silent bug 解消）
