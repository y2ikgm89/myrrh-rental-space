...
| 要素                              | 値                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Section padding                   | DB 駆動セクションは `SectionWrapper` が `layout.padding`(`--space-sm/md/lg/xl`) を適用（`--space-*` は `public.css` fluid スケール）。**content セクションの canonical は `md`(85/48px)**、`lg/xl` は hero/dramatic 専用（連続 padded セクションの `pb+pt` 二重計上回避 → `public-page-gotchas.md §余白の二重計上`）。`SiteCTA` / `related-*` を生む `Section` primitive(`design-system/section.tsx`) の default spacing も `md`、listing/article body 下は `pb-md`（PR #465）                                                                                                                                                                     |
| Footer / 構造的セパレータ padding | `--space-sm` (mobile 32-64px) / `--space-md` (desktop 48-112px) が canonical。`--space-lg / xl` (max 176-240px) は hero / dramatic section 専用、footer / 構造的セパレータには **適用禁止**（業界実測中央値 ~60px、Cereal 最大 100px に対し `--space-xl` max 240px は 2.4 倍で過大）。**業界 footer は fluid scaling せず fixed px が主流**（Cereal / Stripe ともに 1440/1920 で同値）だが、本プロジェクトは editorial decorative 設計として `--space-sm/md` fluid を維持。`site-footer.tsx:168` が参照実装                                                                                                                                        |
...
| Hero                              | ホーム: `pageHero.variant`（editorial-split / compact / minimal / **media**）。旧 `homepage-hero` Section は廃止。**media variant**（2026-05-24 PR #220 で旧 `video` variant + `backgroundImage` の 2 軸分離を統合）: 単一 `media: { url, alt, caption }` フィールド + `detectMediaSourceType(url)` で runtime に image / video 派生 → 画像なら `next/image` 背景描画、動画なら R2 / YouTube / Vimeo を `VideoPlayer variant="background"` で auto-play + loop + mute、`posterImage` は動画選択時の load 中 / autoplay 失敗時 fallback、`overlay` + `overlayOpacity` でテキスト可読性確保。業界標準 WordPress Cover Block / Sanity Studio パターン |
