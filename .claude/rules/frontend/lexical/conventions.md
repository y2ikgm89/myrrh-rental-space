...
12. **React render内でのノードプロパティ直接アクセス禁止**: `editor.read(() => $getState(node, xxxState))` で囲む。Lexicalはアクティブなeditor stateが必要（→ #29、`editor.read()` に統一）
...
21. **`TableCellResizerPlugin` は @lexical/react npm package に存在しない**（playground 専用サンプル実装、0.45.x でも未提供）: 使用禁止。`<TablePlugin hasCellMerge={true} hasCellBackgroundColor={true} />` が現バージョンのテーブル強化の上限
...
28. **Lexical HTML 生成は client (browser) で行う — Server Action から呼ばない** — Next.js 16 の Server Action / RSC は **`react-server` condition** 下で実行され、(1) `react.react-server.js` は `createContext` を export せず（`@lexical/react/LexicalComposerContext` の module-load 時 `createContext(null)` が `createContext is not a function` で破綻）、(2) `react-dom/server.react-server.js` は明示的に `throw new Error('react-dom/server is not supported in React Server Components.')` を実行する（Button / FeatureIconList / InlineIcon の `exportDOM` 内 `renderToStaticMarkup` 経路が死ぬ）。**clean fix**: `renderEditorStateJsonToHtmlClient` を **client editor (submit ハンドラ)** で呼んで事前 render し、Server Action は input で `contentHtml` を受け取るだけにする（→ `prisma-patterns/lexical-storage.md`）。`@lexical/headless/dom` の `withDOM()` は browser では既存 `window` を再利用するだけのため client 内で問題なく動作する。**禁止**: `lazy-renderer.ts` / server-side `headless-renderer.ts` の復活（2026-05-11 削除済、Next.js 16 の `react-server` condition と本質的に非互換）。**update / create command の input schema には `contentHtml: z.string()` を必ず追加** し、client form submit handler で `renderEditorStateJsonToHtmlClient(contentJson)` の結果を渡す（参照実装: `post/mutations.ts` の `updatePostBody` / `usePostEditor.ts` の `onSubmitBody`）
29. **`editor.getEditorState().read(callback)` 単独使用禁止 — `editor.read(callback)` を使う**: 前者は active editor context を establish せず、`$getNearestNodeFromDOMNode` / `$getNearestNodeOfType` 等 `getActiveEditor()` 依存の `$` API が `"Unable to find an active editor"` で fail する silent bug。`$getState(node, state)` は node-keyed で fail しないが新規実装は統一。公式エラーメッセージ自身が `"editor.update(), editor.read(), or editor.getEditorState().read(..., {editor})"` を提示。`{ editor }` 第 2 引数を明示する特殊ケース（別 editor の snapshot を読む等）のみ `editor.getEditorState().read(cb, { editor })` を残す
30. **`<LazyLexicalEditor showInspector={false}>` 禁止 — Lexical を採用したら Inspector ON が canonical**: Inspector OFF だとブロック挿入はできても **property 編集 (画像 alt / 寸法 / Cover overlay / Steps style / Table セル背景 / YouTube アスペクト比 等) ができない** 中途半端な状態になる。WordPress Gutenberg / Sanity Studio / WooCommerce / Eventbrite 全社が Inspector ON 標準。Inspector は default collapsed (`localStorage` `myrrh-lexical-inspector-panel` 未設定 = 0) のため layout 圧迫はユーザー操作時のみで実害なし。**軽量要件 (画像 / 複合 block 一切不要、装飾のみ) なら Lexical ではなく `PortableTextBlockEditor` (`@/admin/components/portable-text/block-editor`、contenteditable + DOM walker、軽量) を採用する** — Lexical を選択した時点で Inspector ON が前提。`showInspector` prop 自体は型レベルで残るが、新規呼び出し側で `false` 指定は anti-pattern (2026-05-28 PR #285 で SpaceEditForm / EventPublishFields の `false` 指定を削除済)
...
- **`MobileEditorFallback`（画面幅 &lt; 1024px）** — 親から渡された **`contentJson` を headless で HTML に変換**してプレビューする（未保存の変更を反映）。**`lexicalJsonSchema` 非適合時はプレビューせず警告**（自動正規化しない）。`EMPTY_LEXICAL_EDITOR_STATE_JSON` は **空段落 1 ブロック**。DB 修正は `.claude/rules/prisma-patterns/lexical-storage.md` §レガシー行の一括修正。headless 変換は `parseEditorState` のあと **`editor.setEditorState(editorState)`** を挟んでから `$generateHtmlFromNodes` すること（省略すると空 HTML になりうる）。実装: `preview/render-editor-state-to-html-client.ts`（client-only canonical、2026-05-11 以降 server-side headless renderer は削除済 → 鉄則 §28）
...
- **Portable Text editor (`PortableTextInlineEditor` / `PortableTextBlockEditor`) は Lexical 不使用 — 責務分離原則** — 2 つの portable-text editor は `_shared/components/portable-text/{inline-editor,block-editor}/` 配下にあるが、内部実装は `contenteditable` div + DOM walker (`serialize-spans.ts` / `serialize-blocks.ts`) で Lexical は import しない。Sanity Portable Text 公式 Span / Block model (`{_key, _type, ...}`) の最小実装で SSR safe + dynamic import 不要 + bundle 軽量。Section の inline label / long-form text 専用 editor として `field.portableTextInline` (Phase 1-3 / span 配列) / `field.portableTextBlock` (Phase 4 / block 配列) で配線され、`AutoSectionForm` の `case "portable-text-inline"` / `case "portable-text-block"` 分岐から呼ばれる。Lexical エディタは Post / News / Terms の rich content 編集に限定（Section の長文 body は config JSON に統合済 — `Section.contentJson` 列廃止 PR #231、現在は `field.portableTextBlock` 経由で `Section.config` JSON 内に保存）。**両者の責務分離**: ① 短い label / 長文 textarea 相当 → portable-text editor（軽量 contenteditable） ② full WYSIWYG / heading / list / table / image / 各種カスタムノード → Lexical。Portable Text editor を Lexical wrapper 化する提案は本責務分離違反のため拒否（Phase 4 plan 作成時の "Lexical wrapper" 文言は誤認識、実装は contenteditable パターンで統一）
- **Inspector パネル UI は Select / RadioGroup を default にする（39 パネル中 38 で統一）** — Lexical Inspector の全 panel（`inspector/panels/*InspectorPanel.tsx`）は **「視覚プリセット選択」「型タグ選択」いずれも Select / RadioGroup が業界標準**（Sanity Studio / Notion / Confluence / Gutenberg と一致）。15+ の prefab を持つ場合でも、カテゴリ別 Select × N（例: GroupNode の border / background / decoration 3 軸）に分けることで、カスタムプレビュー UI なしに完結する。`<Select value="">` は許容（`<SelectItem value="">` のみ Radix UI で禁止）→ カテゴリ外なら placeholder「—」表示で「未選択カテゴリ」を表現可能。視覚プレビュー grid が必要なのは **新規挿入 Dialog 内**のみ（一度きりの選択 = UX 上有利、Figma / Sketch パターン）で、**Inspector の編集 UI には不要**（compact Select で即時切替の方が高速）。新規 InspectorPanel 実装時は default を Select に固定し、prefab を視覚プレビューしたい場合は Dialog 側で対応する。判断ミス回避: 個別パネルの UI 選定前に必ず既存 `inspector/panels/*.tsx` の `Select` / `RadioGroup` 使用パターンを grep で確認する。参照実装: `GroupInspectorPanel`（Select × 3、2026-05-11 統一済）。歴史: 当初 GroupInspectorPanel のみカスタムプレビュー UI で 39 中 1 違反 → ユーザー指摘で Select × 3 に統一
- **`editor.read()` 統一の history** — 2026-05-11 で全 Lexical 配下 54 箇所 / 38 ファイルを `editor.getEditorState().read(cb)` → `editor.read(cb)` に一括移行（公式推奨 API、active editor context 自動 establish）。2026-06-02 に Inspector パネル等で再混入した 19 箇所 / 14 ファイルを再 sweep。一括 regex 置換は `python3` script で実行（`re.sub(r'(\b\w+)\s*\.getEditorState\(\)\s*\.read\(', r'\1.read(', s)` で `\s*` を挟み複数行形式も捕捉、`.toJSON()` / state 受け渡しは非対象）。**検出 grep は複数行対応必須** — 単一行 `grep -rn "getEditorState().read"` は `editor`／`.getEditorState()`／`.read(` が改行分割された chain 形式を取りこぼす。`grep -rPzo "getEditorState\(\)\s*\.read\(" src/`（PCRE multiline）または `rg -U "getEditorState\(\)\s*\.read\(" src/` で残存ゼロを定期チェック
- **インライン補足テキストノードは bare `title` 属性禁止 — focusable + CSS 表示にする** — `<abbr title="...">` は title 属性がモバイル/タッチで表示されず、`<abbr>` も focusable でないためキーボード・タッチユーザーが内容に到達不能（WCAG 2.2 違反）。説明文を **常に DOM 内に保持**（`[role="tooltip"]` 子要素 = スクリーンリーダー可読）+ ホスト `tabindex="0"` + CSS `:hover` / `:focus-visible` / `:focus-within` で視覚表示する。`createDOM` / `decorate` / `exportDOM` を新構造で対称化し、`importDOM` は旧 `title` も graceful fallback で取り込む。`TooltipNode` が参照実装（2026-06-02 刷新）
- **Inspector パネルの Select トリガーを `h-8` で 44px 未満に override 禁止** — Select primitive の既定は `h-11`（44px、WCAG 2.5.5 AAA 準拠）。パネルで `className="h-8 text-sm"` と override すると 32px になり違反する。高さ指定を外し `className="text-sm"` のみにして primitive 既定（44px）に委ねる。装飾アイコンの `h-8 w-8` は対象外（2026-06-02 に 27 ファイル 57 箇所を 44px 化）
