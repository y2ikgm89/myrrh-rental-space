...
- **`editor.read()` 統一の history** — 2026-05-11 で全 Lexical 配下 54 箇所 / 38 ファイルを `editor.getEditorState().read(cb)` → `editor.read(cb)` に一括移行（公式推奨 API、active editor context 自動 establish）。一括 regex 置換は `python3` script で実行（Edit tool 個別呼び出しより 50x 効率的）。検出 grep: `grep -rn "getEditorState\(\)\.read" src/` で残存ゼロを定期チェック
