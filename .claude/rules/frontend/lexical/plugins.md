...
- **ComponentPicker `dialog` entry の起動は `editor.update` の外で同期呼び出し** — `queueMicrotask(() => openDialog(id))` を `editor.update` 内で発火するハックは React state 更新と editor mutation が混在して fragile（DialogRenderer の re-render が取りこぼされ E2E で dialog open 失敗）。canonical: `onSelectOption` 内で item.type 分岐し ① `dialog`: `editor.update(() => { nodeToRemove?.remove(); closeMenu() })` → `openDialog(item.dialogId)` を update 外で同期 ② `command`/`transform`: 全て `editor.update` 内で `applyInsertItemInUpdate(item, editor)`。`applyInsertItemInUpdate` の引数型は `ApplyableInsertItem = CommandInsertItem | TransformInsertItem` で dialog を型レベル排除（2026-05-11 確立、E2E `lexical-inline-icon.spec.ts` で実回帰）
