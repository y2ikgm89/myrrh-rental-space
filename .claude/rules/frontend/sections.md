...
- **schema / DEFAULT に後から追加したフィールドは既存 DB config に反映されない — default が空配列だと CTA 等が silent に機能消失する** — `editorialSplitSchema` に `buttons` を追加 / `DEFAULT_PAGE_HERO.buttons` を拡張しても、既存 `Section.config` に `buttons` キーが無ければ Zod `.default([])` が空配列を補完し、CTA ボタンが**エラーなく非表示**になる（`images` のように fallback 値が描画されないため気付きにくい silent bug。`{buttons.length > 0 && ...}` gate は正しく動作しており、データ欠落が原因）。**対処: 冪等な backfill script で既存 config を補完する**（`scripts/backfill-page-hero-buttons.ts` が参照実装 — `buttons` キーが無い対象 variant のみ `DEFAULT_PAGE_HERO.buttons` を注入、`--dry-run` で対象集計）。section の schema / DEFAULT に新フィールドを足す PR では「既存 DB レコードへ backfill が必要か」を必ず確認する。本番 DB も deploy 後に同 script 実行が運用残（`backfill-oauth-token-encryption.ts` と同じ P2 ops パターン）
