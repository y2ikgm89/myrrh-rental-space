...
- **入力 → ダイアログ起動の trigger は単純 button + 既存 dialog が universal pattern** — cmdk Combobox + Popover のインライン検索は power user 向けで CMS の non-technical user には「trigger に直接タイプできるはず」誤解を生む（実例: 2026-05-10 セッションで cmdk Combobox 採用 → 「UI/UX 何もかもわかりにくい」フィードバック → 単純 button + IconPickerDialog にロールバック）。クリック → 見慣れたカテゴリグリッド dialog が discovery 性高い（`IconPickerField` 参照実装、commit `d40f8319` 44px button + dialog 起動）。slash command (`/icon`) 起動は contenteditable 内の補完として追加可能（`slash-trigger.ts` の `detectSlashIconTrigger` / `replaceTriggerWithElement` が canonical helper、commit `abee2423`）
