...
- **Coupon は `CouponStateToggle` 専用 component を使用** — PublishSwitch.label の binary published/unpublished prop を派生 5 状態（active/inactive/expired/limitReached/notStarted）に流用する API abuse を回避するため専用化。Switch + 派生 state テキスト（セマンティックカラー）の 2 層表示。詳細 → `ssot-ui-components.md` §`CouponStateToggle`
