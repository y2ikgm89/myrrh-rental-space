...
describe("Navigation Admin Action Integration", () => {
  describe("navigationItemSchema バリデーション", () => {
    ...
    describe("label", () => {
      ...
      test("text token 500 文字までは OK（PortableTextSpan max）", () => {
        const result = navigationItemSchema.safeParse({
          ...VALID_NAVIGATION_ITEM_INPUT,
          label: [
            { _key: "k", _type: "span" as const, text: "あ".repeat(500) },
          ],
        });
        expect(result.success).toBe(true);
      });

      test("text token 501 文字はエラー（PortableTextSpan max 違反）", () => {
        const result = navigationItemSchema.safeParse({
          ...VALID_NAVIGATION_ITEM_INPUT,
          label: [
            { _key: "k", _type: "span" as const, text: "あ".repeat(501) },
          ],
        });
        expect(result.success).toBe(false);
      });
      ...
      test("日本語ラベルは許可", () => {
        const result = navigationItemSchema.safeParse({
          ...VALID_NAVIGATION_ITEM_INPUT,
          label: [{ _key: "k", _type: "span" as const, text: "お問い合わせ" }],
        });
        expect(result.success).toBe(true);
      });
    });
  ...
  describe("境界値テスト", () => {
    test("text token 500文字（境界、PortableTextSpan max）", () => {
      const result = navigationItemSchema.safeParse({
        ...VALID_NAVIGATION_ITEM_INPUT,
        label: [{ _key: "k", _type: "span" as const, text: "x".repeat(500) }],
      });
      expect(result.success).toBe(true);
    });

    test("text token 501文字（境界超過、PortableTextSpan max 違反）", () => {
      const result = navigationItemSchema.safeParse({
        ...VALID_NAVIGATION_ITEM_INPUT,
        label: [{ _key: "k", _type: "span" as const, text: "x".repeat(501) }],
      });
      expect(result.success).toBe(false);
    });
