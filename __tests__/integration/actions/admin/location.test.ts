...
const businessTimeSlotSchema = z.object({
  openTime: z
    .string()
    .regex(/^\d{2}:\d{2}$/, { error: '開店時刻は HH:MM 形式で入力してください' }),
  closeTime: z
    .string()
    .regex(/^\d{2}:\d{2}$/, { error: '閉店時刻は HH:MM 形式で入力してください' }),
})
...
const VALID_BUSINESS_HOURS = {
  ...
  wednesday: { isOpen: true, slots: [{ openTime: '09:00', closeTime: '21:00' }] },
  thursday: { isOpen: true, slots: [{ openTime: '09:00', closeTime: '21:00' }] },
  friday: { isOpen: true, slots: [{ openTime: '09:00', closeTime: '22:00' }] },
  saturday: { isOpen: true, slots: [{ openTime: '10:00', closeTime: '22:00' }] },
  ...
  describe('locationFormSchema バリデーション', () => {
    describe('正常系', () => {
      ...
      test('businessHoursはオプション', () => {
        const { businessHours: _bh, ...inputWithoutHours } = VALID_LOCATION_INPUT
      ...
      test('isPublishedのデフォルトはfalse', () => {
        const { isPublished: _ip, ...inputWithoutPublished } = VALID_LOCATION_INPUT
