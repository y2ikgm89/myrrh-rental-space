...
describe('Instagram Admin Action Integration', () => {
  describe('instagramSettingsSchema バリデーション', () => {
    ...
    describe('feedEnabled', () => {
      ...
      test('欠落はエラー', () => {
        const { feedEnabled: _f, ...inputWithoutEnabled } = VALID_SETTINGS_INPUT
  ...
  describe('extractInstagramShortcode ユーティリティ', () => {
    test('投稿URLからショートコードを抽出', () => {
      expect(extractInstagramShortcode('https://www.instagram.com/p/ABC123xyz/')).toBe('ABC123xyz')
    })

    test('リールURLからショートコードを抽出', () => {
      expect(extractInstagramShortcode('https://www.instagram.com/reel/XYZ789abc/')).toBe('XYZ789abc')
    })

    test('末尾スラッシュなしのURLからショートコードを抽出', () => {
      expect(extractInstagramShortcode('https://www.instagram.com/p/TEST123')).toBe('TEST123')
    })

    test('wwwなしのURLからショートコードを抽出', () => {
      expect(extractInstagramShortcode('https://instagram.com/p/SHORTCODE/')).toBe('SHORTCODE')
    })
