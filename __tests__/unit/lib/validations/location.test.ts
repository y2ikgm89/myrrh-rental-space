...
describe('locationFormSchema', () => {
  ...
  test('description が1000文字を超える場合エラーになる', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('説明は1000文字以内で入力してください')
    }
  })
  ...
  test('address が500文字を超える場合エラーになる', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('住所は500文字以内で入力してください')
    }
  })
  ...
  test('access が1000文字を超える場合エラーになる', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('アクセス情報は1000文字以内で入力してください')
    }
  })
  ...
  test('imageUrl が空文字列の場合エラーになる', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('建物画像URLを入力してください')
    }
  })
  ...
  test('imageUrls が10枚を超える場合エラーになる', () => {
    const data = {
      ...
      address: '東京都渋谷区1-2-3',
      imageUrl: 'https://example.com/image.jpg',
      imageUrls: Array(11).fill('https://example.com/img.jpg'),
    }
  ...
  test('businessHours の openTime が HH:MM 形式でない場合エラーになる', () => {
    const data = {
      ...
      imageUrl: 'https://example.com/image.jpg',
      businessHours: {
        monday: { isOpen: true, slots: [{ openTime: '9:00', closeTime: '17:00' }] },
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('開店時刻は HH:MM 形式で入力してください')
    }
  })
  ...
  test('businessHours の closeTime が HH:MM 形式でない場合エラーになる', () => {
    const data = {
      ...
      imageUrl: 'https://example.com/image.jpg',
      businessHours: {
        monday: { isOpen: true, slots: [{ openTime: '09:00', closeTime: '5:00' }] },
        tuesday: { isOpen: false, slots: [] },
        wednesday: { isOpen: false, slots: [] },
        thursday: { isOpen: false, slots: [] },
        friday: { isOpen: false, slots: [] },
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('閉店時刻は HH:MM 形式で入力してください')
    }
  })
