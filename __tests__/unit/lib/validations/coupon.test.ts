...
describe('couponCodeSchema', () => {
  ...
  test('クーポンコードが4文字未満の場合エラーになる', () => {
    ...
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('クーポンコードは4文字以上で入力してください')
    }
  })
  ...
  test('クーポンコードが20文字を超える場合エラーになる', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('クーポンコードは20文字以内で入力してください')
    }
  })

  test('クーポンコードに大文字英数字以外が含まれる場合エラーになる', () => {
    const invalidCodes = [
      { code: 'SAVE-20', expectedError: 'クーポンコードは大文字英数字のみ使用できます' },
      { code: 'SAVE_20', expectedError: 'クーポンコードは大文字英数字のみ使用できます' },
      { code: 'SAVE 20', expectedError: 'クーポンコードは大文字英数字のみ使用できます' },
      { code: 'ABC', expectedError: 'クーポンコードは4文字以上で入力してください' },
      { code: 'save20', expectedError: 'クーポンコードは大文字英数字のみ使用できます' },
      { code: 'セール2024', expectedError: 'クーポンコードは大文字英数字のみ使用できます' },
    ]
...
describe('couponFormSchema', () => {
  ...
  test('name が100文字を超える場合エラーになる', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('名称は100文字以内で入力してください')
    }
  })
  ...
  test('discountValue が0以下の場合エラーになる', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('割引値は0より大きい必要があります')
    }
  })
  ...
  test('パーセント割引の場合、100%を超えるとエラーになる', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      const error = result.error.issues.find((issue) => issue.path[0] === 'discountValue')
  ...
  test('minReservationAmount が負の数の場合エラーになる', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('最低利用金額は0以上で入力してください')
    }
  })
  ...
  test('maxDiscountAmount が0以下の場合エラーになる', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('最大割引額は0より大きい必要があります')
    }
  })
  ...
  test('validUntil が validFrom より前の場合エラーになる', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      const error = result.error.issues.find((issue) => issue.path[0] === 'validUntil')
  ...
  test('usageLimit が整数でない場合エラーになる', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('利用回数上限は整数で入力してください')
    }
  })
  ...
  test('usageLimit が0以下の場合エラーになる', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('利用回数上限は1以上で入力してください')
    }
  })
