...
describe('newCustomerSchema', () => {
  ...
  test('メールアドレスが空の場合にエラー', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toContain('メールアドレスを入力してください')
    }
  })
...
describe('adminReservationSchema', () => {
  ...
  test('スペースIDが無効な場合にエラー', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toContain('スペースを選択してください')
    }
  })
  ...
  test('日付形式が無効な場合にエラー', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toContain('日付の形式が正しくありません')
    }
  })
  ...
  test('時間形式が無効な場合にエラー（開始時間）', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toContain('時間の形式が正しくありません')
    }
  })
  ...
  test('終了時間が開始時間より前の場合にエラー', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues.some((issue) => issue.message.includes('終了時間は開始時間より後'))).toBe(true)
    }
  })
  ...
  test('予約時間が1時間未満の場合にエラー', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues.some((issue) => issue.message.includes('最低1時間以上'))).toBe(true)
    }
  })
