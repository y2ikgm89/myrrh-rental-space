...
import { stripeSettingsSchema, stripeConnectionTestSchema } from '@/admin/lib/validations/stripe'
...
describe('stripeSettingsSchema', () => {
  ...
  test('stripePublishableKey が pk_test_ で始まらない場合エラーになる', () => {
    ...
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('公開可能キーは pk_test_ または pk_live_ で始まる必要があります')
    }
  })
  ...
  test('stripeSecretKey が sk_test_ で始まらない場合エラーになる', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('シークレットキーは sk_test_ または sk_live_ で始まる必要があります')
    }
  })
  ...
  test('stripeWebhookSecret が whsec_ で始まらない場合エラーになる', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('Webhookシークレットは whsec_ で始まる必要があります')
    }
  })
  ...
  test('公開可能キーとシークレットキーのモード（test/live）が一致しない場合エラーになる', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      const error = result.error.issues.find((issue) => issue.path[0] === 'stripeSecretKey')
      expect(error?.message).toBe('公開可能キーとシークレットキーのモード（test/live）が一致していません')
    }
  })
...
describe('stripeConnectionTestSchema', () => {
  ...
  test('secretKey が空文字列の場合エラーになる', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('シークレットキーを入力してください')
    }
  })
