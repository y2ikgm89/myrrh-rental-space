...
describe('getValidReservationStatus', () => {
  test('有効な予約ステータスの場合そのステータスを返す', () => {
    expect(getValidReservationStatus(ReservationStatus.CONFIRMED, ReservationStatus.PENDING)).toBe(
      ReservationStatus.CONFIRMED
    )
  })

  test('無効な予約ステータスの場合フォールバック値を返す', () => {
    expect(getValidReservationStatus('INVALID', ReservationStatus.PENDING)).toBe(
...
describe('getAuditActionFilterOrAll', () => {
  test('有効な監査アクションの場合そのアクションを返す', () => {
    expect(getAuditActionFilterOrAll(AuditAction.CREATE)).toBe(AuditAction.CREATE)
  })
