...
describe('spaceCategoryFormSchema', () => {
  ...
  test('name が空文字列の場合エラーになる', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('カテゴリー名を入力してください')
    }
  })
  ...
  test('name が50文字を超える場合エラーになる', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('カテゴリー名は50文字以内で入力してください')
    }
  })
  ...
  test('icon が50文字を超える場合エラーになる', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('アイコン名は50文字以内で入力してください')
    }
  })
  ...
  test('color が不正な形式の場合エラーになる', () => {
    const invalidColors = ['#fffff', '#ggg', '#123456789', 'red', 'ffffff', '#']
    ...
    for (const color of invalidColors) {
      ...
      expect(result.success).toBe(false)
      if (!result.success) {
        expect(result.error.issues[0].message).toBe('有効なカラーコードを入力してください')
      }
    }
  })
