...
describe('calculateReservationPrice', () => {
  ...
  describe('スペース割引のみ', () => {
    test('パーセント割引が適用される', () => {
      ...
      expect(result.appliedSpaceDiscount).toEqual({ type: 'percentage', value: 10 })
    })
  ...
  describe('長時間割引のみ', () => {
    test('durationDiscountEnabledがtrueの場合、長時間割引が適用される', () => {
      ...
      expect(result.totalPrice).toBe(4500)
      expect(result.appliedDurationRule).toEqual({ hours: 5, discountRate: 10 })
    })
  ...
  describe('併用モード "best"（最もお得な割引のみ適用）', () => {
    test('長時間割引 > クーポン割引の場合、長時間割引のみ適用', () => {
      ...
      expect(result.couponDiscount).toBe(0)
      expect(result.appliedDurationRule).toEqual({ hours: 8, discountRate: 20 })
      expect(result.appliedCoupon).toBeNull()
    ...
    test('bestモードで両方割引ありの場合、警告メッセージが追加される', () => {
      ...
      expect(result.warnings).toContain('より大きな割引が自動的に適用されました')
    })
  ...
  describe('併用モード "both"（両方適用）', () => {
    ...
    test('併用不可クーポンの場合、警告メッセージが追加される', () => {
      ...
      expect(result.warnings).toContain('このクーポンは他の割引と併用できません')
    })
    ...
    test('併用可能で両方適用される場合、情報メッセージが追加される', () => {
      ...
      expect(result.warnings).toContain('長時間割引とクーポン割引が両方適用されています')
    })
  })
...
describe('validateDurationDiscountRules', () => {
  ...
  test('オブジェクトでないルールは無効', () => {
    ...
    expect(result.valid).toBe(false)
    expect(result.error).toBe('各ルールは hours と discountRate を持つ必要があります')
  })
  ...
  test('hoursが数値でないルールは無効', () => {
    const result = validateDurationDiscountRules([{ hours: 'three', discountRate: 5 }])
    expect(result.valid).toBe(false)
    expect(result.error).toBe('各ルールは hours と discountRate を持つ必要があります')
  })

  test('discountRateが数値でないルールは無効', () => {
    const result = validateDurationDiscountRules([{ hours: 3, discountRate: 'five' }])
    expect(result.valid).toBe(false)
    expect(result.error).toBe('各ルールは hours と discountRate を持つ必要があります')
  })

  test('hoursが0以下は無効', () => {
    const result = validateDurationDiscountRules([{ hours: 0, discountRate: 5 }])
    expect(result.valid).toBe(false)
    expect(result.error).toBe('時間は0より大きい必要があります')

    const result2 = validateDurationDiscountRules([{ hours: -1, discountRate: 5 }])
    expect(result2.valid).toBe(false)
    expect(result2.error).toBe('時間は0より大きい必要があります')
  })

  test('discountRateが0未満は無効', () => {
    const result = validateDurationDiscountRules([{ hours: 3, discountRate: -1 }])
    expect(result.valid).toBe(false)
    expect(result.error).toBe('割引率は0〜100の範囲で指定してください')
  })

  test('discountRateが100超は無効', () => {
    const result = validateDurationDiscountRules([{ hours: 3, discountRate: 101 }])
    expect(result.valid).toBe(false)
    expect(result.error).toBe('割引率は0〜100の範囲で指定してください')
  })

  test('discountRateが0は有効', () => {
    const result = validateDurationDiscountRules([{ hours: 3, discountRate: 0 }])
    expect(result.valid).toBe(true)
    expect(result.rules).toEqual([{ hours: 3, discountRate: 0 }])
  })

  test('discountRateが100は有効', () => {
    const result = validateDurationDiscountRules([{ hours: 3, discountRate: 100 }])
    expect(result.valid).toBe(true)
...
describe('parseDurationDiscountRules', () => {
  ...
  test('無効な入力は空配列を返す', () => {
    ...
    expect(parseDurationDiscountRules([{ hours: -1, discountRate: 5 }])).toEqual([])
  })
...
describe('formatPrice', () => {
  ...
  test('税ラベル付きでフォーマットする', () => {
    expect(formatPrice(1000, { showTaxLabel: true, taxLabel: '税込' })).toBe('¥1,000（税込）')
    expect(formatPrice(1000, { showTaxLabel: true, taxLabel: '税抜' })).toBe('¥1,000（税抜）')
  })
  ...
  test('showTaxLabelがfalseの場合、taxLabelは表示されない', () => {
    expect(formatPrice(1000, { showTaxLabel: false, taxLabel: '税込' })).toBe('¥1,000')
  })

  test('全オプション指定', () => {
    expect(
      formatPrice(5000, { showCurrency: true, showTaxLabel: true, taxLabel: '税込' })
    ).toBe('¥5,000（税込）')
  })

  test('通貨記号なし + 税ラベル付き', () => {
    expect(
      formatPrice(5000, { showCurrency: false, showTaxLabel: true, taxLabel: '税抜' })
    ).toBe('5,000（税抜）')
  })
})
