 ...
 * さらに当時の nightly は `cron: "0 18 * * *"` で、15-17 の終端 17:00 と窓の開始が
 * **同時刻**という 0 秒差で通っていた。つまり「たまに落ちる」ではなく
 * **時間帯で決まる恒常的な失敗**だった。
 *
 * 広域 E2E は 2026-08-26 に main への push（マージ直後）へ移したので、実行時刻は
 * **一日のどこにでも来る**。上の 16:00〜18:00 UTC 帯も普通に踏む。時刻で回避する
 * 余地は無くなったので、下の所有分割だけが安全性の根拠になる。
 *
 * ## 何を強制するか
 *
