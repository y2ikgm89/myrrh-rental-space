...
const { permanentlyDeleteExpiredPostTrash } =
  await import("@/shared/domain/posts/trash-commands");

describe("permanentlyDeleteExpiredPostTrash", () => {
  beforeEach(() => {
  ...
  test("保持期間超過の soft-delete 行を deleteMany する", async () => {
    ...
    const result = await permanentlyDeleteExpiredPostTrash(30);
  ...
  test("retentionDays < 0 は VALIDATION", async () => {
    await expect(permanentlyDeleteExpiredPostTrash(-1)).rejects.toMatchObject({
      code: "VALIDATION",
    });
