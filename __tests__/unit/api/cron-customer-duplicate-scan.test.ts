...
const mockDetectDuplicateCandidates = mock<
  () => Promise<{ customerId: string }[]>
>(() => Promise.resolve([]));

const mockApplyDuplicateCandidateFlagsCommand = mock<() => Promise<number>>(
  () => Promise.resolve(0),
);
...
mock.module("@/shared/domain/customers/duplicate-detection", () => ({
  detectDuplicateCandidates: (
    ...args: Parameters<typeof mockDetectDuplicateCandidates>
  ) => mockDetectDuplicateCandidates(...args),
  applyDuplicateCandidateFlagsCommand: (
    ...args: Parameters<typeof mockApplyDuplicateCandidateFlagsCommand>
  ) => mockApplyDuplicateCandidateFlagsCommand(...args),
}));
...
const { GET } = await import("@/app/api/cron/customer-duplicate-scan/route");
...
function makeRequest(authHeader?: string) {
  ...
  return new Request("http://localhost/api/cron/customer-duplicate-scan", {
    headers,
  });
}
...
describe("GET /api/cron/customer-duplicate-scan", () => {
  beforeEach(() => {
    mockDetectDuplicateCandidates.mockReset();
    mockApplyDuplicateCandidateFlagsCommand.mockReset();
    ...
    mockApplyDuplicateCandidateFlagsCommand.mockResolvedValue(0);
  ...
  test("Cloud Scheduler OIDC 認証失敗 → authorizeCronRequest の返却値をそのまま返す (401)", async () => {
    ...
    expect(mockAuthorizeCronRequest).toHaveBeenCalledWith(
      expect.objectContaining({ operation: "customerDuplicateScan" }),
    );
  ...
  test("検知2件 → applyDuplicateCandidateFlagsCommand + createNotificationCommand が呼ばれる", async () => {
  ...
  test("detectDuplicateCandidates が例外をスロー → 500 を返す", async () => {
    const dbError = new Error("Database connection failed");
    mockDetectDuplicateCandidates.mockRejectedValue(dbError);
    ...
    const body = await response.json();
    expect(body).toEqual({ error: "Customer duplicate scan failed" });
