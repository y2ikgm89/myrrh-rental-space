...
const mockPermanentlyDeleteExpiredPostTrash = mock<
...
mock.module("@/shared/domain/posts/trash-commands", () => ({
  permanentlyDeleteExpiredPostTrash: (
    ...args: Parameters<typeof mockPermanentlyDeleteExpiredPostTrash>
  ) => mockPermanentlyDeleteExpiredPostTrash(...args),
}));
...
const { GET } = await import("@/app/api/cron/blog-trash-cleanup/route");
...
function makeRequest(authHeader?: string) {
  ...
  return new Request("http://localhost/api/cron/blog-trash-cleanup", {
    headers,
  });
}
...
describe("GET /api/cron/blog-trash-cleanup", () => {
  beforeEach(() => {
    mockPermanentlyDeleteExpiredPostTrash.mockReset();
    ...
    mockPermanentlyDeleteExpiredPostTrash.mockResolvedValue({ deleted: 0 });
  ...
  test("Cloud Scheduler OIDC 認証失敗 → authorizeCronRequest の返却値をそのまま返す (401)", async () => {
    ...
    expect(mockAuthorizeCronRequest).toHaveBeenCalledWith(
      expect.objectContaining({ operation: "blogTrashCleanup" }),
    );
    expect(mockPermanentlyDeleteExpiredPostTrash).not.toHaveBeenCalled();
  });
  ...
  test("対象なし → deleted=0 で正常終了", async () => {
    ...
    expect(mockPermanentlyDeleteExpiredPostTrash).toHaveBeenCalledWith(30);
  });

  test("対象あり → deleted 件数を返す", async () => {
    mockPermanentlyDeleteExpiredPostTrash.mockResolvedValue({ deleted: 3 });
  ...
  test("permanentlyDeleteExpiredPostTrash が例外をスロー → 500 + logError", async () => {
    const dbError = new Error("Database connection failed");
    mockPermanentlyDeleteExpiredPostTrash.mockRejectedValue(dbError);
