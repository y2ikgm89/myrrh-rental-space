  ...
  return (
    <div className={styles.errorContainer()}>
      <div className={styles.errorCard()}>
        ...
        <Link href="/admin/blog" className={styles.errorLink()}>
          管理画面に戻る
        </Link>
      </div>
    </div>
 ...
 * ブログプレビューコンテンツ
 */
function BlogPreviewContent({ data }: { data: BlogPreviewData }): ReactElement {
  return (
    <>
      <PreviewBanner />
 ...
 */
export function BlogPreviewWrapper({ slug }: BlogPreviewWrapperProps): ReactElement {
  const { data, isPreview } = usePreviewData('blog', slug)
