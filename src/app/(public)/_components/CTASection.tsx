...
function CTAButtons({ primaryButton, secondaryButton, variant }: {
  ...
  return (
    <ScrollReveal delay={0.3}>
      <div className={`mt-8 flex ${isHorizontal ? 'flex-row flex-wrap justify-center' : 'flex-col items-center'} gap-6 md:mt-12`}>
        {primaryButton && (
...
export function CTASection({ config, design }: CTASectionProps): ReactElement {
  ...
  const bgStyle = config.backgroundColor ? { backgroundColor: config.backgroundColor } : undefined
      ...
      <SectionWrapper design={design} style={bgStyle}>
        <div className="flex flex-col items-center gap-8 md:flex-row md:gap-16">
          ...
          <div className="flex shrink-0 flex-col items-center gap-4">
            <CTAButtons primaryButton={primaryButton} secondaryButton={secondaryButton} variant={variant} />
          </div>
        </div>
      </SectionWrapper>
    )
