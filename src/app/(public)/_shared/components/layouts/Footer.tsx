...
export async function Footer(): Promise<ReactElement> {
  ...
  const brandShort = info.name.split(' ')[0]?.toUpperCase() ?? 'MYRRH'
        ...
        <div className="grid gap-10 md:grid-cols-3 md:gap-12">
          ...
          <div>
            ...
            <ul className="mt-4 space-y-3">
              {footerNav.length > 0 ? (
                ...
                <>
                  <li>
                    <Link href="/" className="text-sm text-foreground transition-colors hover:text-primary-dark">
                      Home
                    </Link>
                  </li>
                  <li>
                    <Link href="/reservation" className="text-sm text-foreground transition-colors hover:text-primary-dark">
                      Reservation
                    </Link>
                  </li>
                  <li>
                    <Link href="/contact" className="text-sm text-foreground transition-colors hover:text-primary-dark">
                      Contact
                    </Link>
                  </li>
                </>
          ...
          >
            ...
            <ul className="mt-4 space-y-3 text-sm text-foreground">
              {info.email && (
                <li>
                  <a itemProp="email" href={`mailto:${info.email}`} className="transition-colors hover:text-primary-dark">
                    {info.email}
                  </a>
                </li>
              )}
              {info.phone && (
                <li>
                  <a itemProp="telephone" href={`tel:${info.phone}`} className="transition-colors hover:text-primary-dark">
                    {info.phone}
                  </a>
                </li>
              )}
              ...
              {hoursDisplay.length > 0 && (
                <li className="pt-1">
                  ...
                  <div className="mt-2 space-y-1">
                    {hoursDisplay.map((h) => (
                      <div key={h.microdataContent} className="flex items-center gap-2 text-muted-foreground">
