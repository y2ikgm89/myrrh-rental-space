...
export function AnnouncementBarCarousel({
  ...
  const safeIndex =
    visibleBars.length === 0
      ? 0
      : currentIndex >= visibleBars.length
        ? 0
        : currentIndex;
  ...
  useEffect(() => {
    ...
    // 同一バー or アニメーション中
    if (prevBarIdRef.current === currentBar.id || isAnimatingRef.current)
      return;
  ...
  const goPrev = () => {
    if (visibleBars.length === 0 || isAnimatingRef.current) return;
    setCurrentIndex(
      (prev) => (prev - 1 + visibleBars.length) % visibleBars.length,
    );
  };
  ...
  const needsDefaultText =
    !hasCustomText &&
    (designStyle === AnnouncementBarDesignStyle.solid ||
    ...
    <>
      ...
      <div
        ...
        className={cn(
          ...
          !hasCustomText &&
            (designStyle === AnnouncementBarDesignStyle.outlined ||
              designStyle === AnnouncementBarDesignStyle.minimal) &&
            "text-foreground",
