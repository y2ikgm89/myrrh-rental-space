...
function createParticles(
  ...
  for (let i = 0; i < count; i++) {
    ...
    const size = sizeRange[0] + deterministicHash(seed + 2) * (sizeRange[1] - sizeRange[0])
    const alpha = opacityRange[0] + deterministicHash(seed + 3) * (opacityRange[1] - opacityRange[0])
...
export function PixiParticleSprites({
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      ...
      particlesRef.current = createParticles(count, width, height, sizeRange, opacityRange)
