 ...
 * 利用規約ページSEO設定フォーム
 *
 * サイト全体の利用規約ページ（/terms）のSEO/OGP設定を編集
 */
...
import { updateSiteWideTermsSeo } from '@/admin/actions/terms'

interface TermsSeoFormProps {
  /** 現在のSEO設定 */
  seoData: SiteWideTermsSeo
}

export function TermsSeoForm({ seoData }: TermsSeoFormProps) {
  ...
  const {
    ...
    control,
    formState: { errors },
  } = useForm<UpdateTermsSeoInput>({
    resolver: zodResolver(updateTermsSeoSchema),
    defaultValues: {
  ...
  const onSubmit = async (data: UpdateTermsSeoInput) => {
    startTransition(async () => {
      const result = await updateSiteWideTermsSeo(data)
      if (result.success) {
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label>ページタイトル</Label>
            <Input value={seoData.title} disabled />
