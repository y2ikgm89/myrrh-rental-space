    ...
    <form onSubmit={handleSubmit((data) => onSave({ config: data }))} className="space-y-6">
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <Input id="news-section-label" {...register('sectionLabel')} placeholder="例: News" disabled={isPending} />
        </div>

        <div className="space-y-2">
          <Label htmlFor="news-title">タイトル</Label>
          <Input id="news-title" {...register('title')} placeholder="お知らせ" disabled={isPending} />
          {errors.title && <p className="text-sm text-destructive">{errors.title.message}</p>}
        </div>

        <div className="space-y-2">
          <Label htmlFor="news-max">表示件数</Label>
          <Input id="news-max" type="number" min={1} max={20} {...register('maxItems', { valueAsNumber: true })} disabled={isPending} />
        </div>
        ...
        <div className="flex items-center gap-2">
          <Switch id="news-view-all" checked={config.showViewAllLink} onCheckedChange={(checked) => setValue('showViewAllLink', checked)} disabled={isPending} />
        ...
        {showViewAllLink && (
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <Input id="news-view-all-text" {...register('viewAllText')} placeholder="全てのお知らせ" disabled={isPending} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="news-view-all-url">「全て見る」リンク先</Label>
              <Input id="news-view-all-url" {...register('viewAllUrl')} placeholder="/news" disabled={isPending} />
            </div>
          </div>
        )}
      </div>
