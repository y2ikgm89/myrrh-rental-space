    ...
    <form onSubmit={handleSubmit((data) => onSave({ config: data }))} className="space-y-6">
      <div className="space-y-4">
        <div className="space-y-2">
          <Label htmlFor="post-section-label">セクションラベル（英語装飾）</Label>
          <Input id="post-section-label" {...register('sectionLabel')} placeholder="例: Blog" disabled={isPending} />
        </div>

        <div className="space-y-2">
          <Label htmlFor="post-title">タイトル</Label>
          <Input id="post-title" {...register('title')} placeholder="最新の記事" disabled={isPending} />
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-2">
            ...
            <Input id="post-max" type="number" min={1} max={20} {...register('maxItems', { valueAsNumber: true })} disabled={isPending} />
          </div>
          <div className="space-y-2">
            <Label htmlFor="post-columns">カラム数</Label>
            <Input id="post-columns" type="number" min={1} max={4} {...register('columns', { valueAsNumber: true })} disabled={isPending} />
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <Switch id="post-view-all" checked={config.showViewAllLink} onCheckedChange={(checked) => setValue('showViewAllLink', checked)} disabled={isPending} />
        ...
        {showViewAllLink && (
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <Input id="post-view-all-text" {...register('viewAllText')} placeholder="全ての記事" disabled={isPending} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="post-view-all-url">「全て見る」リンク先</Label>
              <Input id="post-view-all-url" {...register('viewAllUrl')} placeholder="/posts" disabled={isPending} />
            </div>
          </div>
        )}
      </div>
