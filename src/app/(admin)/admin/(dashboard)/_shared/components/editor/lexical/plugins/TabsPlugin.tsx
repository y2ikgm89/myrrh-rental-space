...
export function handleTabClick(
  ...
  if (tabList) {
    ...
    for (const title of titles) {
      $setState(
        title,
        tabTitleActiveState,
        $getState(title, tabTitleIndexState) === clickedIndex,
      );
    }
  }
  ...
  for (const panel of panels) {
    $setState(
      panel,
      tabPanelActiveState,
      $getState(panel, tabPanelIndexState) === clickedIndex,
    );
  }
...
export function $addTab(container: TabsContainerNode): TabPanelNode {
  ...
  panelParagraph.append(
    $createTextNode(`タブ${newIndex + 1}のコンテンツを入力`),
  );
...
export function $removeTab(
  ...
  const newActive =
    index <= currentActive ? Math.max(0, currentActive - 1) : currentActive;
...
export function $reorderTab(
  ...
  if (currentActive === fromIndex) {
    ...
    if (currentActive > fromIndex && currentActive <= toIndex)
      newActive = currentActive - 1;
  } else {
    if (currentActive >= toIndex && currentActive < fromIndex)
      newActive = currentActive + 1;
  }
...
export function TabsPlugin({ isOpen, onClose }: TabsPluginProps) {
  ...
  const [selectedFixedWidth, setSelectedFixedWidth] =
    useState<TabsFixedWidth>("120");
  ...
  const handleInsert = () => {
    ...
    editor.update(() => {
      const tabsContainer = $createTabsContainerNode(
        0,
        selectedStyle,
        selectedSize,
        selectedFixedWidth,
