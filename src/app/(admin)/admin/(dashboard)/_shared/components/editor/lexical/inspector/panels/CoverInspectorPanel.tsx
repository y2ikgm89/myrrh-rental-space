...
export function CoverInspectorPanel({ nodeKey, node }: CoverInspectorPanelProps) {
  const [editor] = useLexicalComposerContext()
  const updateNode = useNodeUpdater(nodeKey, $isCoverNode)

  const backgroundImageUrl = editor.getEditorState().read(() =>
    $getState(node, backgroundImageUrlState)
  )
  const overlayColor = editor.getEditorState().read(() =>
    $getState(node, overlayColorState)
  )
  const overlayOpacity = editor.getEditorState().read(() =>
    $getState(node, overlayOpacityState)
  )
  const minHeight = editor.getEditorState().read(() =>
    $getState(node, minHeightState)
  )
  const contentAlign = editor.getEditorState().read(() =>
    $getState(node, contentAlignState)
  )
  const contentPosition = editor.getEditorState().read(() =>
    $getState(node, contentPositionState)
  )

  const handleBgUrlChange = (value: string) => {
    updateNode((n) => { $setState(n, backgroundImageUrlState, value) })
  }

  const handleOverlayColorChange = (value: string) => {
    if (isAccentColor(value)) {
      updateNode((n) => { $setState(n, overlayColorState, value) })
    }
  }
          ...
          >
            {COVER_CONTENT_ALIGNS.map((align) => (
              <div key={align} className="flex items-center space-x-1.5">
                <RadioGroupItem value={align} id={`inspector-cover-align-${align}`} />
                <Label
          ...
          >
            {COVER_CONTENT_POSITIONS.map((pos) => (
              <div key={pos} className="flex items-center space-x-1.5">
                <RadioGroupItem value={pos} id={`inspector-cover-position-${pos}`} />
                <Label
