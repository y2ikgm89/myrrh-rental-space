...
export function PricingFeatureInspectorPanel({
  nodeKey,
  node,
}: PricingFeatureInspectorPanelProps) {
  ...
  const handleIncludedChange = (checked: boolean) => {
    updateNode((n) => {
      $setState(n, featureIncludedState, checked);
    });
  };
    ...
    <div>
      ...
      <InspectorSection title="表示設定">
        <div className="flex items-center justify-between">
          <Label htmlFor="inspector-feature-included" className="text-xs">
            含まれる
          </Label>
          <Switch
