...
export function TabsInspectorPanel({ nodeKey, node }: TabsInspectorPanelProps) {
  ...
  const handleStyleChange = (value: string) => {
    if (isTabsStyle(value)) {
      updateNode((n) => {
        $setState(n, tabsStyleState, value);
      });
    }
  };
  ...
  const handleSizeChange = (value: string) => {
    if (isTabsSize(value)) {
      updateNode((n) => {
        $setState(n, tabsSizeState, value);
      });
    }
  };
  ...
  const handleFixedWidthChange = (value: string) => {
    if (isTabsFixedWidth(value)) {
      updateNode((n) => {
        $setState(n, tabsFixedWidthState, value);
      });
    }
  };
  ...
  const handleAddTab = () => {
    editor.update(() => {
      ...
      const newIndex =
        container.getChildren().filter($isTabPanelNode).length - 1;
