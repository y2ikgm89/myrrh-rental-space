  ...
  return {
    ...
    latestComment: latestComment
      ? {
          ...
          createdByName: latestComment.createdByUser?.name ?? "不明",
        }
      : undefined,
 ...
 * コメントを追加（返信）
 */
export const addComment = withPermission<[AddCommentInput], EditorComment>(
  "post",
  "update",
)(async (user, input) => {
...
>("post", "read", { audit: false })(async (_user, query) => {
  ...
  try {
    ...
    const userIds = [
      ...new Set(
        [
          ...threads.map((t) => t.createdBy),
          ...threads.flatMap((t) => t.comments.map((c) => c.createdBy)),
    ...
    const items = threads.map((thread) => ({
      ...
      createdByUser: thread.createdBy
        ? (userMap.get(thread.createdBy) ?? null)
        : null,
      comments: thread.comments.map((comment) => ({
        ...comment,
        createdByUser: comment.createdBy
          ? (userMap.get(comment.createdBy) ?? null)
          : null,
      })),
    }));
