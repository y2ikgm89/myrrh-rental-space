...
export type CouponData = {
  ...
  /** React 19 Server→Client シリアライズ済み ISO 8601 文字列 */
  validFrom: string;
  /** React 19 Server→Client シリアライズ済み ISO 8601 文字列 */
...
/** CouponFormOutput → Prisma create/update data */
function toPrismaData(data: CouponFormOutput) {
  return {
    code: data.code,
    name: data.name,
...
export async function getCoupons(
  ...
  const sortOrder = ALLOWED_SORT_ORDER.has(rawSortOrder)
    ? rawSortOrder
    : "desc";
  ...
  if (search) {
    ...
    if (existingOr && Array.isArray(existingOr)) {
      // 既存のOR条件とANDで結合
      where["AND"] = [{ OR: existingOr }, { OR: searchCondition }];
      delete where["OR"];
    } else {
 ...
 * クーポンを新規作成
 */
export const createCoupon = withPermission<
 ...
 * クーポンを更新
 */
export const updateCoupon = withPermission<
  [id: string, input: CouponFormInput],
  void
>(
  "coupon",
  "update",
...
export const toggleCouponActive = withPermission<[id: string], void>(
  ...
  return createSuccess(
    coupon.isActive ? "クーポンを無効化しました" : "クーポンを有効化しました",
  );
});
