...
import { recordManualReservationPayment } from "@/admin/actions/reservation";
...
export function RecordManualPaymentDialog({
  ...
  const handleSubmit = async () => {
    ...
    if (amountNum !== defaultAmount) {
      setError(
        `金額は${defaultAmount.toLocaleString()}円と一致する必要があります。`,
      );
      return;
    }
    ...
    setIsPending(true);
    const result = await recordManualReservationPayment({
      reservationId,
      amount: amountNum,
      method,
