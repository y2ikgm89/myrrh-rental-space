 ...
 * `getSeriesGcalMasterEventId` は domain 層 (calendar-sync-series.ts) 経由で実データを
...
import "server-only";
import { markReservationCalendarSyncUpdated } from "@/shared/domain/reservations/calendar-sync";
import {
 ...
 * （`calendar-sync-series.getSeriesGcalMasterEventId` への委譲）。
 ...
 *
 * flow:
 *   1. domain (calendar-sync-series.getSeriesForCalendarSync) から現行 series の RRULE と
