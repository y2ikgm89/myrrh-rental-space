        ...
        <div className={cn('space-y-1 text-muted-foreground', styles.breakdown)}>
          {appliedDurationRule && durationDiscount > 0 && (
            <div className="flex items-center gap-2">
              <span className="text-blue-600">長時間割引</span>
              <span>
          ...
          {appliedCoupon && couponDiscount > 0 && (
            <div className="flex items-center gap-2">
              <span className="text-green-600">クーポン</span>
...
export function CompactDiscountPrice({
  ...
  return (
    <span className={cn('inline-flex items-baseline gap-1', className)}>
      ...
      <span className={cn('rounded bg-red-500 px-1 text-white', size === 'sm' ? 'text-[10px]' : 'text-xs')}>
        {actualRate}%OFF
      </span>
    </span>
  )
