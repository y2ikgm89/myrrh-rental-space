...
/**
 * 規約プレビュー URL を生成（Next.js 16 server-side preview パターン）。
 * `(public)/preview/terms/[id]/page.tsx` が DB から `getTermsByIdForPreview(id)` で
 * 最新 draft を fetch する。
 */
export function getTermsPreviewHref(id: string): string {
  return `/preview/terms/${id}`;
}
