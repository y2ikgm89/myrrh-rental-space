 ...
 * SwitchBot / Instagram Graph API / Google Cloud Run
 *
 * オンライン決済は Stripe Checkout のみを前提とし、当サイトに存在しない
 * 支払い手段（非 Stripe 銀行振込・請求書払い等）をテンプレートに盛り込まない。
 *
 * ## プレースホルダー
 *
...
<li>未成年者（満 18 歳未満の者）が本サービスを利用する場合は、事前に法定代理人（親権者等）の同意を得なければなりません。同意の確認ができない、または同意がないと当社が判断した場合、当社は予約のお断り・取消し等の措置を講じることがあります。法令上お客様に認められる権利は、当該法令の定めに従います。</li>
</ol>
...
<li>予約は、当社のシステムにおいて予約申込みを受け付け、予約確認通知がされた時点をもって成立します。オンライン決済機能が有効な場合、Stripe Checkout による支払いは予約成立後に行われることがあり、未払いの予約についてはサイト設定に基づく支払期限が適用される場合があります。</li>
...
<li>料金の詳細、支払方法、返金等については「支払い規約」に従うものとします。オンライン決済は Stripe, Inc. の Stripe Checkout を通じて処理されます。</li>
...
<li>法定代理人の同意が確認できない、または同意がないと当社が判断した場合、当社は予約のお断り・取消し等の措置を講じることがあります。</li>
<li>法定代理人の同意なく本サービスを利用した場合の責任は、利用者および法定代理人が負うものとします。法令上お客様に認められる権利は、当該法令の定めに従います。</li>
</ol>
...
<li><strong>Cloudflare R2</strong>：メディアファイルの保存。公開用バケット（スペース画像・ロゴ・オープングラフ画像等の運営コンテンツ配信）と、お問い合わせ添付ファイル用の非公開バケット（アップロードされたファイルに個人情報が含まれる場合があります）を用途別に利用します。</li>
</ul>
<ul>
...
<h3>7.11 スマートロック（SwitchBot）</h3>
<p>施設への入室管理に、SwitchBot 社のスマートロック連携機能を利用しています。予約に紐づく時間限定のゲスト用パスコードは、当社システム上で暗号化して保管し、予約確認メール等によりお客様へお知らせします。キャンセル・利用終了・有効期限経過等に応じて、パスコードは削除または無効化されます。</p>
...
${callout(
  "info",
  "<p>ご予約のキャンセルには、予約画面に表示される返金ポリシー（ご利用開始日時を基準とした返金率）に基づき、返金されない部分がキャンセル料として発生します。キャンセル時期が利用日に近いほど返金率が低くなる場合があります。お早めのお手続きにご協力ください。</p>",
)}
...
<h2>2. キャンセル料・返金率</h2>
<p>キャンセル時の返金額は、予約画面・マイページ等に表示される返金ポリシー（ご利用開始日時の何時間前かに応じた返金率）に従います。返金ポリシーは当社のサイト設定により管理され、キャンセル時期はシステム上のキャンセル受付完了時刻を基準に判定します。</p>
<p>返金率が 100% 未満の場合、返金されない部分がキャンセル料（お客様のご負担）となります。返金ポリシーが設定されていない場合、既にお支払い済みの金額については全額返金を原則とします（決済手数料等の取扱いは支払い規約に従います）。</p>

${callout(
  "warning",
  "<p><strong>ご利用当日のキャンセルおよび無断キャンセル（ノーショー）は、返金ポリシー上 0% 返金となる場合があります。</strong>予定が変わった場合は、できるだけ早めにキャンセルのお手続きをお願いいたします。</p>",
)}

<h2>3. キャンセル方法</h2>
<ol>
<li>キャンセルは、マイページの「予約」より該当予約を選択してお手続きください。</li>
...
<h2>5. 返金</h2>
<ol>
<li>返金額は、第 2 条の返金ポリシーに基づき算出し、主として Stripe, Inc. の決済サービス「Stripe」を通じ、ご利用時と同一のオンライン決済手段（クレジットカード、コンビニ決済、Stripe 銀行振込（Cash Balance）、Link 等、当時の決済方法に応じます）へ返金します。返金処理の完了までに、決済手段により 5〜30 日程度を要することがあります。</li>
...
<h2>6. 予約変更について</h2>
<ol>
<li>未払い（UNPAID）の予約については、サイト設定および空き状況の範囲内で、マイページから日時・内容等の変更（同一予約の編集）が可能な場合があります。</li>
<li>オンライン決済の開始後、または支払い完了後の予約内容の変更は、原則としてキャンセルのうえ新規予約として承ります。キャンセル時の返金・キャンセル料は、第 2 条の返金ポリシーに従います。</li>
...
<p>お客様側の事情による天災・交通機関の運休・感染症罹患等を理由とするキャンセルにつきましても、本ポリシー第 2 条の返金ポリシーに従います。必要に応じて旅行保険等のご利用をご検討ください。</p>

<h2>9. ノーショー・繰り返しキャンセル</h2>
<ol>
<li>無断キャンセル（ノーショー）は、返金ポリシー上 0% 返金となる場合があります。</li>
...
<h2>4. 支払方法</h2>
<p>オンライン決済は、Stripe, Inc. の「Stripe Checkout」を通じて提供されます。利用可能な決済手段は、当社のサイト設定により異なり、以下のいずれか（または複数）が有効化されている場合にご利用いただけます。</p>
<ul>
<li><strong>クレジットカード</strong>（VISA / Mastercard / JCB / American Express / Diners Club 等）</li>
<li><strong>コンビニ決済</strong></li>
<li><strong>Stripe 銀行振込（Cash Balance）</strong></li>
<li><strong>Link</strong>（Stripe Link）</li>
</ul>
<p>上記以外の決済手段は、本サービスの予約決済には提供しません。</p>

<h2>5. 決済代行会社（Stripe）</h2>
<ol>
<li>オンライン決済は、Stripe, Inc.（アメリカ合衆国）の提供する決済サービス「Stripe Checkout」を通じて処理されます。</li>
<li>Stripe は PCI DSS（Payment Card Industry Data Security Standard）Level 1 に準拠した決済基盤を提供しており、クレジットカード情報等は Stripe のサーバー上で直接取得・管理されます。当社のサーバーにはクレジットカード情報は保存されません。</li>
<li>Stripe の利用規約およびプライバシーポリシーについては、https://stripe.com/jp/legal をご確認ください。</li>
</ol>

<h2>6. 支払時期</h2>
<ol>
<li>予約申込み時点では、原則として決済は不要です。予約は当社システムによる受付・確認通知をもって成立し、未払い（UNPAID）の状態で登録されます。</li>
<li>オンライン決済機能が有効で、利用料金が 0 円超の場合、お客様は予約成立後、マイページ等から Stripe Checkout にてお支払いいただけます。ゲスト予約の場合は、所定の手続きによるアカウント連携（予約の引き取り）後に決済できる場合があります。</li>
<li>サイト設定により、支払期限が定められている場合、期限内にお支払いがない予約はキャンセルされることがあります。</li>
</ol>
...
<h2>8. 返金</h2>
<p>返金が発生する場合の取扱いは、「キャンセルポリシー」に従います。決済代行手数料等は返金対象外とする場合があります。</p>
...
<li>入室方法（SwitchBot スマートロックの解錠パスコード・鍵の受渡し等）は、予約確認メールまたはマイページにてご案内します。</li>
...
<tr><th>電話受付時間</th><td>${PLACEHOLDER.phoneHours}</td></tr>
...
<h2>支払方法</h2>
<p>Stripe Checkout により、当社のサイト設定で有効化された以下の決済手段からお選びいただけます。</p>
<ul>
<li>クレジットカード（VISA / Mastercard / JCB / American Express / Diners Club 等）</li>
<li>コンビニ決済</li>
<li>Stripe 銀行振込（Cash Balance）</li>
<li>Link（Stripe Link）</li>
</ul>

<h2>支払時期</h2>
<ul>
<li>予約申込み時点では、原則として決済は不要です。</li>
<li>オンライン決済機能が有効で利用料金が 0 円超の場合、予約成立後、マイページ等から Stripe Checkout にてお支払いいただけます。</li>
<li>サイト設定により支払期限が定められている場合、期限内にお支払いがない予約はキャンセルされることがあります。</li>
</ul>
...
const COOKIE_POLICY_TEMPLATE: TermsTemplate = {
  ...
  label: "Cookie ポリシー",
  description:
    "Cookie および類似技術の利用に関する独立ポリシー（電気通信事業法外部送信規律・当サイトの運用に準拠）",
  content: `${LAST_UPDATED_BLOCK}
...
${callout(
  "info",
  "<p>当サイトは、ログイン状態の維持等の必須 Cookie と、アクセス解析等の非必須 Cookie を利用しています。Cookie 同意バナーでは、アクセス解析等の非必須 Cookie の読み込み可否をお選びいただけます。本ポリシーでは、利用する Cookie の種類・第三者への外部送信・無効化の方法をご説明します。</p>",
)}
...
<p>本サービスの基本機能の提供に不可欠なため、Cookie 同意バナーの選択にかかわらず利用します。無効化すると本サービスが正常に利用できなくなります。</p>
<ul>
...
<h3>3.2 アクセス解析 Cookie（同意に基づき利用）</h3>
<p>本サービスの利用状況の分析、改善に利用します。Cookie 同意バナーで同意いただいた場合にのみ読み込みます。</p>
<ul>
<li><strong>Google Analytics</strong>：ページ閲覧数、滞在時間、流入経路等の匿名統計情報の取得</li>
</ul>

<h3>3.3 マーケティング Cookie（同意に基づき利用）</h3>
...
<p>これらの外部送信のうち、必須 Cookie に該当しないもの（アクセス解析等）については、Cookie 同意バナーでの選択、または本ポリシー第 6 条記載の方法により Cookie を無効化することで送信を停止できます。</p>
