...
export async function Hero(): Promise<ReactElement> {
  ...
  // ヒーローが非アクティブの場合は何も表示しない
  if (!hero.isActive) {
    return <></>
  }

  const hasBackgroundImage = !!hero.backgroundImageUrl

  return (
    <section className={cn(variants.section())}>
      {/* Background image (if set) */}
      {hasBackgroundImage && hero.backgroundImageUrl && (
        <>
      ...
      <Container className={cn(variants.content())}>
        <h1 className={cn(hasBackgroundImage ? variants.headingWithBg() : variants.heading())}>
          {hero.title}
        </h1>

        {hero.subtitle && (
          <p className={cn(hasBackgroundImage ? variants.subheadingWithBg() : variants.subheading())}>
            {hero.subtitle}
          </p>
        )}
        ...
        <div className={cn(variants.buttonGroup())}>
          ...
          {hero.ctaSecondaryText && hero.ctaSecondaryUrl && (
            <Link
              ...
              className={cn(buttonVariants({ variant: hasBackgroundImage ? 'secondary' : 'outline', size: 'lg' }))}
            >
