...
- **管理画面 form は conform `useActionState` + `executeConformMutation` canonical** — React Hook Form (`react-hook-form` / `@hookform/resolvers`) は `package.json` から完全削除済、新規利用不可。Server Action は `(prev, formData) => SubmissionResult` signature、id 必要時は `bind` 部分適用、`executeConformMutation` SSoT helper (`@/shared/lib/forms/conform-action`) 経由で `executeAdminMutationResult` を呼ぶ。**Page 遷移 form の成功時遷移は server-side `redirect(toAppRoute(...))`** — client `router.push` 不要、created id 取得 → server redirect で詳細ページへ。動的配列は ① **`form.insert/remove/reorder` + `getFieldList()` + `getFieldset()`** (Zod schema 駆動、LocationForm canonical で dnd-kit + `form.reorder({ name, from, to })`) または ② **`useState<{key, ...}[]>` + 安定 key + hidden input append + schema preprocess** (SpaceEditForm canonical、imageUrls `string[]` / facilities `JSON[]` の MediaPicker / IconPickerField 連携時、`crypto.randomUUID()` key で React reconciliation + dnd-kit 整合) のいずれかを採用。**In-place schema preprocess** で canonical schema を object literal (test) と FormData transit (conform) 両対応にする (LocationForm / SpaceEditForm canonical、backward compat 維持)。**conform fields の sub-component 流用は type SSoT** (`Required<{[K in keyof z.input<typeof schema>]: FieldMetadata<...>}>`、EventForm 確立)。**Pass-through JSON state** は hidden input `JSON.stringify` transit + schema `z.preprocess(JSON.parse, ...)` (UI 編集なし複雑 nested object)。**PortableTextSpan[] は hidden input + `JSON.stringify` transit (Pattern B)** + schema 内で `z.string().transform(JSON.parse).pipe(...)` で server-side parse (`useInputControl` bridge より simple、contenteditable cursor 問題回避)。**Lexical contentJson + contentHtml は browser 側 `renderEditorStateJsonToHtmlClient` 派生計算** (React Compiler 自動メモ化、`useMemo` / `flushSync` 不要、batching 問題回避、TermsForm / SpaceEditForm / Posts / News canonical、`react-server` condition 非互換のため Server Action での render 禁止 → `prisma-patterns/lexical-storage.md`)。**5+ tab の大型 form は monolithic 単一 file** (LocationForm / SpaceEditForm canonical、1100-1800 行規模、tab 分割 + prop drilling より maintainable、React Compiler が中規模 component のメモ化を自動処理)。conform Zod 4 integration は `@conform-to/zod/v4` 専用 subpath (root は Zod v3 用で非互換)。inline editor (Posts / News) は本文 useState + 設定 conform `useForm` の dual pattern (`usePostEditor` / `useNewsEditor` 参照)（→ `frontend/admin-ui/forms.md` §Server Actions の認証パターン / `frontend/admin-ui/dialogs.md` §canonical pattern Variant A / `frontend/admin-inline-editor-patterns.md`）
