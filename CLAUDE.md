...
- **公開ページのセクション背景色交互配置禁止** → 全コンテンツセクション `bg-background`（白）統一。`bg-surface` はフッターのみ。セクション分離は余白 + `border-t`。画像プレースホルダは `bg-card` → `project-design-config.md`
...
公開ページ: Editorial Magazine（Kinfolk/Cereal）— シャープエッジ、serif/sans 対比、bronze accent ≤10%、editorial ボタン（シャープエッジ + bronze hover） → `project-design-config.md`
公開ページ: ページ固有 UI と同種の DB セクション（`faq-list`, `cta` 等）は `trailingSections` から除外必須 → `gotchas.md` §Page-First Architecture
...
bun run test                                  # 全テスト（39バッチ分離実行: mock.module 干渉回避）
...
| `src/app/(public)/spaces/_components/image-carousel.tsx` | スペースカード内画像カルーセル（ホバー時左右ナビ、モバイルスワイプ、ドットインジケーター）。`SpaceCard` が `imageUrls` 2枚以上で自動使用                                                                             |
