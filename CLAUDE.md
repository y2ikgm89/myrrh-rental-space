...
- **管理画面 form は conform `useActionState` + `executeConformMutation` canonical** — Phase 1 Task 4-7 で確立。Server Action は `(prev, formData) => SubmissionResult` signature、id 必要時は `bind` 部分適用、`executeConformMutation` SSoT helper (`@/shared/lib/forms/conform-action`) 経由で `executeAdminMutationResult` を呼ぶ。動的配列は `form.insert.getButtonProps` / `form.remove.getButtonProps` + `getFieldList()` + `getFieldset()` で操作 (RHF `useFieldArray` 等価、Zod schema 駆動)。`useFormAction` (RHF) / `standardSchemaResolver` / `react-hook-form` 新規利用禁止 (Task 8 で `package.json` から削除予定)。conform Zod 4 integration は `@conform-to/zod/v4` 専用 subpath (root は Zod v3 用で非互換)。**参照実装**: settings sections 17/17 完了 (PR #61 9 件 / PR #62 medium 7 件 / #74-83 / #84 DiscountSection useFieldArray→`form.insert/remove` / #85 TwoWaySync / #86 Stripe masked input + cross-field refine / #87 GoogleCalendar composite reminder mode)、Dialog 内 PoC (PR #64) / AdminDetailLayout 編集ページ dual-impl (PR #68) / Dialog Variant A mount-on-open + bind 部分適用 (PR #88 taxonomy managers)（→ `frontend/admin-ui/forms.md` §Server Actions の認証パターン (conform canonical) / `frontend/admin-ui/dialogs.md` §canonical pattern Variant A）
