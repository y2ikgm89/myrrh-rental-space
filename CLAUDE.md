...
- **datetime-local は `formatDateTimeLocalInJst` / `parseDateTimeLocalAsJst`、`@db.Date`（日付のみ）は `parseJstDateOnly` / `formatJstDateOnly`** — `new Date(localStr)` は tz 依存で 9h ずれる silent bug（`@db.Date` は「JST カレンダー日付を UTC 深夜保持」、`date-format.ts` SSoT）
