...
- **`'use cache'` を含む domain query の integration test は test env 制約で skip 判断、unit + extended integration で代替カバー** — `cacheLife is only available with the cacheComponents config` (Next.js 16 dev 制約、`bunfig` で `cacheComponents` 反映不可) で `__tests__/integration/<resource>/<query>.test.ts` 経由の `'use cache'` 関数呼び出しが 500。新規 integration dir 作成せず ① pure builder branches を unit test (`__tests__/unit/lib/<area>/<builder>.test.ts`) ② Server Action validation を既存 `__tests__/integration/actions/admin/<resource>.test.ts` に extend で同等カバー（実例: 2026-04-27 MEO Phase 1 T8 で `__tests__/integration/domain/locations/jsonld-data.test.ts` 不採用、unit 13 + extended 17 cases で代替）
...
- **Plan の修正対象 file リストは `grep -rln "<symbol>" src/` で実体検証してから書く** — Settings field 削除等の cascade 系 plan は caller を全列挙しないと「plan 外ファイル並行修正」が justified deviation として頻発する。caller 数を plan の Files 節に記載して implementer の cascade 修正を「plan 範囲」に含める（実例: MEO Phase 1 Task 2 で `settings.{latitude\|longitude\|priceRange\|...}` 8 consumer を plan 4 件記載 → 並行修正 4 件追加で commit が膨張）
...
- **ファイル削除時の dangling ref 検出範囲は `docs/` 全域 + `.claude/` + `AGENTS.md` + `CLAUDE.md` 必須** — `.claude/skills/<>` だけでなく `docs/architecture/*.md` 内に dangling link が紛れていることあり（C4 で `ARCHITECTURE.md` の `guides/nuqs.md` / `DATABASE_DESIGN.md` の `guides/type-safety.md` + `PRISMA_7_IMPORT_GUIDE.md` / `.claude/skills/audit-adr-drift/SKILL.md` の violation 例 link を検出）。検出時は「削除 + dangling ref 修正」を同一 commit に統合する
- **Plan 記載の component prop / type 仕様も `ls + grep` で実在確認必須** — Plan に書かれた `<Button asChild>` / `businessInfo.googleMapsUrl` / `logError` from `@/shared/lib/errors` 等が、実装に存在しない API（公開 `Button` Primitive は `asChild` 未対応 / `BusinessInfo` 型に `googleMapsUrl` 無し / `logError` は `@/shared/lib/errors/server` 経由必須）だった silent bug が一度に複数発生する（実例: MEO Phase 1 Task 5 の page.tsx テンプレ Write 直後に validate で 5 型エラー TS2305/TS2322/TS2339 が同時発火、各々 plan template の lift 漏れ）。Plan の destination URL 実在確認（既存 learning）と同列で、実装着手前に ① 公開 Component の prop 表（`grep -nE "interface .+Props" <component-file>`）② import 元 export（`grep -nE "^export" <module-file>`）③ 型 field 一覧（`grep -A20 "interface BusinessInfo" <type-file>`）の 3 軸を ground truth として確認する
- **Plan の Task が別 plan / 既存実装で対処済の場合 no-op 判定が正しい完遂** — C3 (skills-cleanup, no-op plan) や 2026-04-28 C5 Phase 2 (Task 2.2/2.3 が `2026-04-28-agents-cleanup.md` で既対処済) のように、implementer 投資で事前完了が判明したら no-op 報告 + controller verify + commit 数が spec range 下回るのは valid pattern。spec range 14-23 → 実績 8 commit (C5) のような乖離は no-op 詳細を commit message と handoff memory に記録すれば許容。implementer は「やらなくていい」を BLOCKED でなく DONE_WITH_CONCERNS で報告
- **ADR README index と ADR ファイル数の整合 grep** — `ls docs/architecture/decisions/00[0-9][0-9]-*.md | wc -l` を `grep -c "^| \[00" docs/architecture/decisions/README.md` + 1（template 1 件分）と比較。差分があれば README に未追加の新 ADR がある drift（C4 で 0022 を検出 → commit `a33ad80f` で追加）
- **Plan 内の helper / API 名は rule docs 記述を盲信せず実装 grep で照合** — `.claude/rules/<>.md` が stale 化している場合あり。Plan 作成段階で `grep -rln "^export.*<helper>" src/` で実在確認、不在なら rule docs 自体を後追い codify する。実例: 2026-04-27 P16 plan で `auth-patterns.md` 記述から `createSuccess` / `createFailure` を `@/admin/types/server-actions` から import 前提で書いたが実装に export なし、subagent が `createMutationError` で justified deviation を出して凌いだ
- **同パターン phase 連続実行時は前 phase の reactive fix を新 plan に proactive 記載必須** — Phase 1 で 1 度 reactive fix した issue（例: cloudflare partial mock の batch pollution → Phase 1 commit `aebc3052` で全 11 export stub 化）を Phase 2 plan の「Files to create」例コードに最初から組み込むと、Phase 2 dispatch で再発ゼロにできる。判定基準: 「Phase 1 で fix したコミットメッセージが `test(*):` / `fix(*): * stub` 等の reactive 文言」なら Phase 2 plan に proactive 移植する。controller の plan 設計責務（実例: P19 Phase 2 plan で cloudflare 全 stub テンプレを Bundle D/E/F すべてに先行記載 → 6 dispatch すべて mock pollution ゼロ）
- **同パターン phase 2 以降の spec / plan は「Phase N を ground truth とした差分形式」推奨** — 同型実装の Phase 2+ で Phase 1 の spec / plan を全文コピーすると冗長。`> **Spec**: Phase 1 spec を ground truth として、差分のみ記述` を冒頭に明記し、領域固有の差分テーブル + 共通 architecture/test/禁止事項は「Phase 1 から継承」と省略する。実例: P19 Phase 2 spec / plan が Phase 1 (530 行) → Phase 2 (350 行) で同等情報量 + dispatch prompt 効率化
- **`revise-claude-md` 引数で「クリーン実装」「ベストプラクティス」「全面 audit」が混入した場合の two-track 分離** — 本来の skill フロー（セッション学習を CLAUDE.md に反映、track A）と引数タスク（track B）を分離。track B 着手前に `~/.claude/projects/<slug>/memory/MEMORY.md` で同 scope の既存 handoff (`*-handoff.md`) の有無を確認し、重複があれば「既存 handoff として記録済み、新セッションで brainstorming → writing-plans → subagent-driven-development が canonical」と通知して track A のみ進める。track B を本セッションで強行すると CLAUDE.md「revise-claude-md はセッション終了直前に呼ぶ」（プロンプトキャッシュ層）+「context 圧迫時は session 跨ぎ handoff の判断を controller 側で行う」と二重に衝突する（実例: 2026-04-27 P19 Phase 3 完了後の C5 handoff 重複検出）

### Git / Migration

- **Worktree 作成前 3 軸チェック**: `git status --short | wc -l` + `ls prisma/migrations/ | tail -1` + `bunx --bun prisma migrate status` で**未コミット migration / DB drift / 未適用 migration** を検出。drift / 未適用あれば先に main で `db execute --file` + `migrate resolve --applied` を実施してから worktree 作成（実例: 2026-04-27 MEO Phase 1 worktree で先行未適用 2 件を implementer が手動 resolve するハメに）（→ `gotchas.md`）
...
- **ADR Superseded 化と関連 dangling references cleanup を同一 commit で完了必須** — ADR の Status を Superseded に変更し scripts を削除しても、`.claude/hooks/*.sh` (hook trigger) / `.claude/settings.json` (hook entry) / `.claude/settings.local.json` (permission) / 関連 rule docs (workflow 言及) / 関連 skill (cross-reference table) に参照が残存して silent fail を引き起こす。canonical sweep: `grep -rln "<deprecated-name>" .claude/hooks/ .claude/settings*.json .claude/rules/ .claude/skills/ docs/architecture/ CLAUDE.md AGENTS.md`（実例: 2026-04-28 C5 Phase 1 で `verify-policy-docs.mjs` (commit `583f3955` で削除済) の dangling 5 箇所を Finding 1 として一括 cleanup、commit `64dcc622`）
...
- **ADR 採番は worktree 内の未 merge feature ブランチも cross-check 必須** — main の `ls docs/architecture/decisions/` だけでは別 worktree のローカルブランチで予約済みの番号を見落とす。`for w in .worktrees/*; do echo "$w:"; (cd "$w" && ls docs/architecture/decisions/ 2>/dev/null | grep "^00"); done` で並走 feature の採番状況を確認してから plan に書く。Plan 段階で X と書いても merge 順番次第で衝突して rename commit が後追いで発生する（実例: 2026-04-27 P16 で `feature/multi-location-seo-foundation` の 0023 と衝突 → 0024 リナンバー commit `d0b175ab`）
...
- **memory file（`~/.claude/projects/<slug>/memory/*.md`）の連続 Edit は auto-format race で失敗する** — `Edit` 直後に別の `Edit` を発行すると「File has been modified since read, either by the user or by a linter」エラー。system-reminder が変更後の内容を inline 表示するため、その内容を ground truth として残り Edit を再構築する（`Read` 再実行は不要、system-reminder が代替）。Edit を 5+ 件並行発行する場合は 1 件ずつ順次完了を確認する規律が必要
- **diverged worktree branch の merge は `--no-ff` 推奨**（FF 不可時） — `git rev-list --count main..feature/X` が N、`git rev-list --count feature/X..main` が M（>0）で diverged の場合、rebase より `--no-ff` merge + conflict 解決の方が作業量同等で history が明示的になる（merge commit が「並行開発の境界」を示す）。CLAUDE.md 既存規律「Linear history は `--ff-only`」は **diverge していない場合**の規律で、diverged は `--no-ff` 採用が proper。実例: 2026-04-27 セッションで P19 Phase 1+2 完了後の P16 + MEO worktree merge を `--no-ff` で実施 (commit `410f44a0` / `5220433c`)
- **並行 worktree merge の典型 conflict 箇所は ADR README + `getCacheTag` namespace** — 複数 worktree が独立に新規 ADR エントリを追加すると `docs/architecture/decisions/README.md` の表に同位置 conflict、独立に新規 namespace（`auditLogs` / `locations` 等）を `src/shared/lib/constants/cache.ts` の `getCacheTag` object に追加すると同位置 conflict。canonical resolution: ADR README は **採番順で並べる**（0023 → 0024）、cache.ts は **両方の追加を残す**（同 object literal 内に並列追加）。実例: 2026-04-27 P16/MEO merge で実発生
- **Windows `.worktrees/` ディレクトリの強制削除失敗は harmless** — `git worktree remove --force` がファイル名長エラー（例: `Filename too long`）で disk 上の dir 削除に失敗しても、`git worktree prune` + `git branch -d feature/X` で **git references はクリーンアップ済み**。`git worktree list` から消えていれば後続作業に影響なし。disk 残骸は完全削除する場合のみ PowerShell `Remove-Item -Recurse -Force '.worktrees\<name>'` で対応（dev / build に影響しない無害な残骸）
...
- **bulk action plan の標準 file 構造**（参照: `docs/superpowers/plans/2026-04-27-admin-bulk-actions-phase{1,2}.md`） — リソースごとに **5 files create**（`src/shared/domain/<resource>/bulk-commands.ts` + `src/app/(admin)/admin/(dashboard)/_shared/actions/<resource>/bulk.ts` + `_components/<Resource>BulkActions.tsx` + `__tests__/unit/domain/<resource>/bulk-commands.test.ts` + `__tests__/integration/actions/admin/<resource>-bulk.test.ts`）+ **1-2 files modify**（`<Resource>Table.tsx` 行 checkbox 追加 / `<Resource>TableHeader.tsx` all-select）+ **`package.json` 追記不要**（既存 `__tests__/unit/domain/<resource>` / `__tests__/integration/actions/admin` directory batch で自動取り込み）。Bundle ごとに 1 commit、3 並列 dispatch 可能（リソース独立のためファイル衝突なし）
...
- **Per-slug cache invalidation 配線は domain command 戻り値拡張駆動** — `updateXCommand` / `createXCommand` の戻り型を `{ id }` → `{ id, slug }` 拡張し、Server Action `afterSuccess(data) => updateTag(getCacheTag.X.detail(data.slug))` で per-slug + ベースタグ両方を invalidate。Server Action の `MutationResult<T>` 戻り型 + test mock fixture (`__tests__/unit/domain/<r>/commands.test.ts` の `mockResolvedValue`) も同一 commit で同期 cascade 必須。slug 取得のための afterSuccess 内追加 query は禁止（execute 戻り値で完結、`gotchas/domain.md` §`afterSuccess` データ受け渡し原則と整合）。実例: Location.slug invalidation (commit `b5238aa6`)
- **Reactive form sub-card（score card / live preview）は Form 内部 Tabs で `form.control` 共有** — page.tsx に Tabs を配置 + Form / SubCard を別子要素にすると `FormProvider` 必須で複雑化。Tabs を Form component 内部に配置（`<Form>{<Tabs>{<TabsContent value="basic">{form fields}</>}{<TabsContent value="meta">{<SubCard control={form.control} />}</>}</Tabs>}</Form>` 構造）すれば `form.control` を直接子に渡せて `FormProvider` 不要。`forceMount` + `data-[state=inactive]:hidden` で Radix Tabs の SC children preservation も維持（`gotchas/prohibitions.md` §Radix Tabs と整合）。実例: LocationForm + LocationMeoScoreCard (commit `b5238aa6`)
...
- **Skill / dir rename を implementer dispatch で扱う場合は git mv 禁止 + 新パス Write + 旧パス rm + controller commit で git auto-detect** — `git mv` も `git add/commit` 全面禁止の dispatch では、implementer が ① 旧 dir のファイルを Read ② 新 dir に Write (`name:` 等の identifier フィールドも新名に更新) ③ 旧 dir を `Bash: rm path` で削除 (`git rm` 禁止) で実装。controller が `git add -A` + `git commit` 時に git の rename detection (95%+ similarity) が auto 検出して `rename` として記録 (実例: 2026-04-28 C5 Phase 3 commit `8b9e9188` で 13 skill rename + reference/ subdir も 100% similarity rename として記録)
- **完了済み plan / spec は `docs/superpowers/{plans,specs}/.archive/<year>/` 配下に保管** — 各 archive file 冒頭に `> **Snapshot: YYYY-MM-DD** — Implementation completed, archived as historical reference.` 追記。`docs/superpowers/{plans,specs}/README.md` で active vs archive 分離を明示。git rename detection (99% similarity) で履歴保持。判定基準: ① plan 内 commit SHA が main で実在 ② plan の最終 task 完了済 ③ 実装が main に存在 (実例: 2026-04-28 C5 Phase 4 で 10 plan + 6 spec を archive 移動、commit `ff4d81c8`)
...
- **Server Action `redirect(externalUrl)` の typedRoutes cast** — Next.js 16 `typedRoutes: true` 環境で外部 OAuth URL（`getGbpAuthorizeUrl()` / `getInstagramAuthorizeUrl()` 等の Google/Meta 等が返す動的 URL）を `redirect()` に渡す場合、`redirect(url as Route<string>)` の library boundary cast が必要（`router.push` ClickableTableRow パターンと同列）。`import type { Route } from "next"` は型のみ import で `"use server"` ファイル制約と非衝突（async 関数のみ export 規律を破らない）。実例: `actions/settings/google-business-profile.ts` の `initiateGbpAuth`
- **50+ 行・複数 describe block の範囲削除は Edit tool より Python regex.sub が信頼性高い** — Edit は old_string の正確マッチを要求するため、66 行以上の multi-line block（test の `describe(...)` block 全体 200+ 行等）は special char escape / 改行ノーマライズ / formatter 介入で一致失敗しやすい。`python3 -c "import re; ...; pattern = re.compile(r'<start_marker>.*?<end_marker>', re.DOTALL); new_text, count = pattern.subn('', text); ..."` で範囲指定削除する方が安全 + 1 回で済む（実例: MEO Phase 1 Task 6 で `__tests__/integration/actions/admin/settings-business.test.ts` の `describe("meoSettingsSchema バリデーション")` block 200+ 行を Python regex で 1 block 削除成功）。判定基準: ① 削除範囲が 50 行超 ② 開始・終了の独自 marker（コメント or describe 名）あり ③ 単一 file 単一 block — 3 条件揃えば Python regex.sub に切り替える
- **Plan 記載の型 contract（`select` clause / `interface` fields）は implementer 独自判断で削減しない** — JSON-LD / SEO / UI 価値に影響する重要 field（`businessHours` / `amenities` / `specialHolidays` 等の SEO data source）を plan の型から省くと後段 Task で出力欠落の cascade を引き起こす。implementer は型を最小化する場合 BLOCKED でも DEVIATION でもなく **controller への確認** で escalate する。controller dispatch prompt に「型定義は plan 通り保持、削減判断は controller に escalate」を明記（実例: MEO Phase 1 Task 2 で `LocationForSeo` から `businessHours` を省いた結果、Task 3 builder が `openingHoursSpecification` 出力を欠いた簡易版になり Task 5 で再判断必要に）
- **handoff memory の「次セッション判断ポイント」は controller 判断で plan 範囲外の追加 chore commit を bundle 化** — 前セッションが「軽量実装 → 次セッション判断」と残した課題は、新セッションで本体 Task 着手前に **Task X.5 として独立 chore commit** を bundle に挟むのが最速。plan 改修 + handoff 更新で吸収するより implementer dispatch が 1 回で済む。実例: MEO Phase 1 で `LocationForSeo` 軽量化を Task 3.5 で拡張（commit `6974e655`）→ Task 4（commit `f792d5ab`）を 1 dispatch sequential で完了
- **subagent dispatch prompt に path alias の解決先を 1 行明記** — `@/admin/*` は `tsconfig.json` で `./src/app/(admin)/admin/(dashboard)/_shared/*` に解決されるため、subagent が plan のコード例の `_shared/X` 接頭辞を見て `@/admin/_shared/X` と二重 prefix で import 文を書く事故あり。dispatch prompt に「import alias は `@/admin/*` / `@/public/*` / `@/shared/*` の 3 系統、いずれも `_shared/` 配下にマップ。`@/admin/_shared/X` は二重 prefix で誤り」を 1 行明示するのが最小コスト（実例: 2026-04-27 P16 全 13 commit の dispatch で都度注記が発生）
- **enum cascade refactor の Bundle 設計** — 共通リソース（`enums/helpers.ts` の状態遷移マップ等）は最初の Bundle に同梱して並列 dispatch 時の同一ファイル race を回避、後続 Bundle は読み取り専用に。implementer の dispatch を sequential bundle にする場合も「共通リソース → 各リソース固有」の順を保つ（実例: 2026-04-27 P19 Phase 3 Bundle G commit `74ce7305` で `CUSTOMER/INQUIRY/EVENT_STATUS_TRANSITIONS` 3 マップ集約 → H/I は読み取りのみ）
- **Bulk status change command の戻り値型は `{ count, newStatus, affectedIds, rejectedIds }`** — Phase 1/2 の `{ count, isActive, affectedIds }` の superset。状態遷移マップ違反は early throw せず `rejectedIds` に積み bulk 自体は valid な ID のみ処理（非破壊）。UI で「N 件は遷移不可のためスキップ」表示。reservation の単発 `validateStatusTransition` は throw でよいが bulk は早期 throw 禁止（→ `enums/helpers.ts` `RESERVATION_STATUS_TRANSITIONS` 参照、実例: P19 Phase 3 `bulkSetStatus<Customer\|Inquiry\|Event>Command`）
- **Bulk email 関数の SSoT 参照実装** — `sendEventCancelledToAllParticipants` (`@/shared/lib/email/event-emails.ts`) が canonical: `prisma.findMany` で対象取得 → `Promise.allSettled` 並列送信 → 失敗 per-item `logError`（category: `EXTERNAL_API`, severity: `MEDIUM`）→ bulk 自体は成功扱い。idempotency key は `<event-type>/<entity-id>(/<variation>)`。新規 bulk email 追加時は必ず複製（→ `external-api-retry-patterns.md`、実例: 2026-04-27 P19 Phase 3 で `sendInquiryStatusNotificationToAll` が同パターンで追加）
- **rule docs vs 実装 drift 解消は実装 SSoT で rule docs 追従** — `.claude/rules/**` の helper / 型 / import が src/ 不在（架空）で実装が別 helper / 型で動作している場合、3 案 (a) helper を src/ 実装 / (b) rule docs を実装追従 / (c) 関係整理 のうち **(b) が最短経路**。production 動作中の実装が canonical のため。ADR 不要（rule docs 内部 drift、public API breaking なし）、1 commit で完結（中間 drift 状態作らない）、`grep -rnE "<forbidden>" .claude/rules/` 0 件で verification。実例: 2026-04-28 Finding 2 で `createSuccess` / `createFailure` / `ActionResult` を `MutationResult<T>` / `createMutationError` に統一（commit `199994fe`、8 files +108/-130）
...
- **2000+ 行 plan の Read 戦略** — controller が full Read すると `.claude/rules/**` path-scoped auto-load と相まって context が破裂する。Task ごとに `Read offset/limit` で 200-300 行ずつ読み、implementer には plan の path を渡して該当 Task のみ Read させる（subagent-driven-development skill の "Make subagent read plan file" red flag は意図的に許容）。controller は plan 各 Task の commit message + Files 節 + 重要 step のみ事前確認すれば十分（実例: MEO Phase 1 plan 2411 行を Task 1-3 で逐次 Read し context 維持）
...
- **`subagent-driven-development` skill invoke + worktree 内 file Read の同時発火は context 二重圧迫** — skill content（implementer-prompt.md 等、合計 ~30K chars）と worktree 配下 `.claude/rules/**` の path-scoped autoload（`gotchas/` / `frontend/` / `react/` / `zod-patterns.md` 等）が同ターンで system-reminder 注入される。controller は **skill invoke 前に「本セッションで実装まで完遂可能か」予算判断**し、不可なら 1 Task のみ dispatch + 残 Task は新セッション handoff の方針を先に決める。実例: 2026-04-27 MEO Phase 1 で skill content + 大量 rule auto-load が 60K+ tokens 消費し Task 5-9 を同セッションで実行不可となった
- **controller inline 実装でも path-scoped rule auto-load の累積消費を予算管理する** — 単一セッション内で `src/app/(admin)/admin/(dashboard)/_shared/actions/settings/**` / `src/shared/domain/**` / `src/app/(public)/_shared/components/**` 等を跨いで連続編集すると、path-scoped で `admin-ui-patterns.md` / `prisma-patterns.md` / `server-actions/**` / `tailwind-patterns.md` / `frontend/**` / `gotchas/**` 等が chain auto-load されて 1 ターンで 100K+ token 消費する。Task 単位で「touch するファイル群が同一 path-scope に収まるか」を事前判定し、跨ぐなら **Task 完了 commit ごとにセッション分割を検討**（実例: MEO Phase 1 Task 5 + 6 を同セッションで完遂したが、Task 6 終盤で Edit / Read / Bash の各レスポンスに 6-7 個の rule docs が同時注入され Task 7 着手不能になった）。subagent-driven-development の context 二重圧迫対策（既存 learning）と同様の規律を controller inline 実装にも適用する
- **controller context が path-scoped auto-load で圧迫された後は残 task を bundle dispatch で context isolation を取る** — Task 7 の controller inline 実装で大量 rule docs auto-load → Task 8+9 を 1 implementer bundle で 2 commit 完遂し controller は git verify のみ。subagent fresh context が rule auto-load を再吸収しても controller は影響を受けない。判定基準: 残 task の実装行数 < 1500 行 + plan の commit 分割が明確（per-task commit message が plan に明記）+ controller の path-scoped 残量 < 30%。「予算管理」learning の concrete escape hatch（実例: 2026-04-27 MEO Phase 1 で `b5238aa6` 後の context 圧迫を `1efd90d2` + `a77b471c` の 2 commit を 1 dispatch で吸収）
- **Phase plan の path-scoped auto-load 予算は 4 領域跨ぎで判定** — `src/shared/lib/validations/enums/*` + `src/app/(admin)/.../_shared/actions/*` + `_components/*` + `__tests__/**` の 4 領域跨ぎ Plan は ~100K tokens auto-load を発火し controller inline 完遂不可。**Sequential-commit plan は 1 implementer bundle dispatch + controller Bundle 別 commit** 戦略を最初から採用し、implementer には git 全面禁止（編集のみ）+ Bundle 別 commit message を plan に明記。controller は完了報告後に Bundle 別ファイル群を `git add` で分離 stage して 3 commit 作成（実例: 2026-04-27 P19 Phase 3 で 18 files / ~1700 行を 1 dispatch 完遂、controller が Bundle G `74ce7305` / H `c4aff8ff` / I `39142b66` の 3 commit に分割）
...
- **handoff memory の完了マーカーは Snapshot + Completed 併記 + Completion Summary + 持ち越し findings 構造** — `> **Snapshot: YYYY-MM-DD**` (発行時) + `> **Completed: YYYY-MM-DD**` (完了時) を併記し、`## Completion Summary` セクションに phase 別 commit 数 + 持ち越し findings (別 plan で扱う drift 等) を記録して archive 候補マーク。MEMORY.md index も同期更新 (実例: 2026-04-28 `project_clean-break-c5-handoff.md` で C5 完了 + Finding 2 持ち越しを記録)
...
- **research/analysis または完了済み project handoff の memory は冒頭に `> **Snapshot: YYYY-MM-DD**` を入れる** — `/audit-memory-staleness` skill で自動履歴扱いされる。clean-break refactor 完了 memory（「廃止済みパターン」「削除済みファイル」等の意図的履歴参照を含む）も対象。ADR で supersede された場合は併せて `> **Superseded: YYYY-MM-DD** — ADR XXXX で置換` を追記。filename に `YYYY-MM-DD` を含む場合も同等
