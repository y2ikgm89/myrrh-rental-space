...
- **管理画面 form は conform `useActionState` + `executeConformMutation` canonical** — Phase 1 Task 4-6 で確立。Server Action は `(prev, formData) => SubmissionResult` signature、id 必要時は `bind` 部分適用、`executeConformMutation` SSoT helper (`@/shared/lib/forms/conform-action`) 経由で `executeAdminMutationResult` を呼ぶ。`useFormAction` (RHF) / `standardSchemaResolver` / `react-hook-form` 新規利用禁止 (Task 8 で `package.json` から削除予定)。conform Zod 4 integration は `@conform-to/zod/v4` 専用 subpath (root は Zod v3 用で非互換)。参照実装: PR #61 settings 9 件 / PR #62 medium 7 件 / PR #64 Dialog 内 PoC / PR #68 AdminDetailLayout 編集ページ dual-impl（→ `frontend/admin-ui/forms.md` §Server Actions の認証パターン (conform canonical)）
