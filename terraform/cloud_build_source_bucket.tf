...
# **runner の bucket 権限が前提。これが無いと本番デプロイが全て止まる。**
#
# 2026-08-30 の最初の試み（PR #2793）は、runner が bucket metadata を読めず
# `terraform plan` が
#
#   Error 403: ... does not have storage.buckets.get access to the Google Cloud
#   Storage bucket
#
# で落ち、`Terraform Apply (IAM prereq)` の失敗で以降のジョブが全 skip された。
# PR #2804 で revert し、PR #2805 で `scripts/bootstrap-terraform.sh` 7b に
# custom role `terraformRunnerBucketMetadataOnly`
# (`storage.buckets.get` + `storage.buckets.update` のみ) を用意して bucket-scope
# で付与してから戻したのがこのファイル。
#
# **付与済みであることは Policy Troubleshooter で確認する**（runner の
# impersonate は F1 structural closure により project owner でもできない）:
#
#   gcloud policy-troubleshoot iam \
#     "//storage.googleapis.com/projects/_/buckets/${PROJECT_ID}_cloudbuild" \
#     --principal-email=terraform-runner@${PROJECT_ID}.iam.gserviceaccount.com \
#     --permission=storage.buckets.get
#
# 実測 2026-08-30: get / update とも `GRANTED`。
#
# **`terraform validate` はこれを検知できない**（backend も credentials も使わない）。
# 新しい種類のリソースを足すときは、runner が触れるかを先に確かめること。
#
# -----------------------------------------------------------------------------
