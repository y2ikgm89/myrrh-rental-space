...
test.describe('Reservation Flow', () => {
  ...
  test('should load reservation page with space selection prompt when no spaceId', async ({
    ...
    await expect(
      page.getByRole('heading', { name: /予約するスペースを選択してください/i })
    ).toBeVisible()
    await expect(page.getByRole('link', { name: /スペース一覧を見る/i })).toBeVisible()
  })

  test('should load reservation form when valid spaceId is provided', async ({ page }) => {
  ...
  test('should allow selecting date and time slots', async ({ page }) => {
    ...
    // Select start time
    const startTimeSlot = page.locator('button', { hasText: validStartTime }).first()
    await expect(startTimeSlot).toBeVisible()
    await startTimeSlot.click()

    // Select end time
    const endTimeSlot = page.locator('button', { hasText: validEndTime }).first()
  ...
  test('should validate required fields on customer info form', async ({ page }) => {
  ...
  test('should validate email format', async ({ page }) => {
    ...
    await page.locator('input[name="lastName"]').fill(testReservations.valid.customerName)
  ...
  test('should validate phone number format', async ({ page }) => {
    ...
    await page.locator('input[name="lastName"]').fill(testReservations.valid.customerName)
    await page.locator('input[name="firstName"]').fill('太郎')
    await page.locator('input[name="email"]').fill(testReservations.valid.customerEmail)
    await page
      .locator('input[name="phoneNumber"]')
  ...
  test('should complete valid reservation and show success message', async ({
    ...
    await context.route('**/*turnstile*', (route) => route.fulfill({ status: 200, body: '{}' }))
    ...
    await expect(page.getByText(/予約を受け付けました/i)).toBeVisible({ timeout: 10000 })
    await expect(
  ...
  test('should prevent duplicate booking for the same time slot', async ({
    ...
    await page.locator('input[name="firstName"]').fill('太郎')
    await page.locator('input[name="email"]').fill(`first-${Date.now()}@example.com`)
    ...
    await newContext.route('**/*turnstile*', (route) => route.fulfill({ status: 200, body: '{}' }))
    ...
    const bookedStartTime = newPage.locator('button', { hasText: validStartTime }).first()
    ...
    if (!isDisabled && !hasDisabledClass) {
      ...
      await newPage.locator('button', { hasText: validEndTime }).first().click()
      ...
      await newPage.locator('input[name="email"]').fill(`second-${Date.now()}@example.com`)
      await newPage.locator('input[name="phoneNumber"]').fill('090-2222-2222')

      const newTermsCheckbox = newPage.locator('input[type="checkbox"]#agreedToTerms')
  ...
  test('should allow going back to date/time selection from customer info', async ({ page }) => {
    // Navigate to customer info step
    await page.goto(urls.spaces)
    const spaceLinks = page.locator('a[href*="/spaces/"]')
    await spaceLinks.first().click()
  ...
  test('should display selected reservation details on customer info step', async ({ page }) => {
    // Navigate to customer info step
    await page.goto(urls.spaces)
    const spaceLinks = page.locator('a[href*="/spaces/"]')
    await spaceLinks.first().click()
