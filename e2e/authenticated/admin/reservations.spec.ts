...
test.describe("予約一覧ページ", () => {
  test("予約一覧ページが正しく表示される", async ({ page }) => {
    ...
    // 説明文を確認
    await expect(
      page.locator("text=予約の確認・ステータス変更・キャンセル処理を行います"),
    ).toBeVisible();
  ...
  test("予約がテーブルに表示される", async ({ page }) => {
    ...
    // テーブルヘッダーを確認
    await expect(
      page.locator("th").filter({ hasText: "予約日時" }),
    ).toBeVisible();
    await expect(
      page.locator("th").filter({ hasText: "スペース" }),
    ).toBeVisible();
  ...
  test("予約がない場合に空の状態が表示される", async ({ page }) => {
    // 存在しない検索クエリで空の状態を確認
    await page.goto(
      `${urls.adminReservations}?search=nonexistent-reservation-12345`,
    );
    await page.waitForLoadState("networkidle");
...
test.describe("予約ステータスの変更", () => {
  ...
  test("一覧ページでステータスを変更できる", async ({ page }) => {
    ...
    await expect(
      page.locator('[role="option"]:has-text("保留中")'),
    ).toBeVisible();
    await expect(
      page.locator('[role="option"]:has-text("確認済み")'),
    ).toBeVisible();
    await expect(
      page.locator('[role="option"]:has-text("キャンセル")'),
    ).toBeVisible();
  });
  ...
  test("詳細ページでステータスを変更できる", async ({ page }) => {
    ...
    // セレクトオプションが表示されることを確認
    await expect(
      page.locator('[role="option"]:has-text("保留中に変更")'),
    ).toBeVisible();
    await expect(
      page.locator('[role="option"]:has-text("確認済みに変更")'),
    ).toBeVisible();
    await expect(
      page.locator('[role="option"]:has-text("キャンセルに変更")'),
    ).toBeVisible();
  });
  ...
  test("ステータスバッジが正しく表示される", async ({ page }) => {
    ...
    const statusBadges = page.locator(
      '[data-testid="status-badge"], .rounded-full',
    );
...
test.describe("検索機能", () => {
  ...
  test("検索クエリがURLに反映される", async ({ page }) => {
    ...
    await page.waitForURL(
      new RegExp(
        `search=${encodeURIComponent(testReservations.valid.customerName)}`,
      ),
      { timeout: 5000 },
    );
  });
  ...
  test("URLの検索クエリが検索フィールドに反映される", async ({ page }) => {
    ...
    await page.goto(
      `${urls.adminReservations}?search=${encodeURIComponent(searchTerm)}`,
    );
    await page.waitForLoadState("networkidle");
  ...
  test("検索とステータスフィルターを組み合わせて使用できる", async ({
    page,
  }) => {
    await page.goto(urls.adminReservations);
    await page.waitForLoadState("networkidle");
...
test.describe("カレンダー表示", () => {
  test("カレンダー表示ボタンをクリックするとカレンダーページに遷移する", async ({
    page,
  }) => {
    await page.goto(urls.adminReservations);
    await page.waitForLoadState("networkidle");
  ...
  test("カレンダーページが読み込まれる", async ({ page }) => {
    ...
    const pageContent = page.locator(
      'main, [role="main"], .calendar, [data-testid="calendar"]',
    );
