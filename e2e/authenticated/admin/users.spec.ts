...
test.describe("Admin Users Management", () => {
  ...
  test.describe("Users List Page", () => {
    test("should display users list page with correct title", async ({
      page,
    }) => {
      ...
      // Verify description
      await expect(
        page.getByText("管理者とユーザーアカウントを管理"),
      ).toBeVisible();
    });
    ...
    test("should display users table with correct headers", async ({
      page,
    }) => {
      await page.goto(urls.adminUsers);
      await waitForPageLoad(page);
      ...
      await expect(
        page.locator("th").filter({ hasText: "ロール" }),
      ).toBeVisible();
      await expect(
        page.locator("th").filter({ hasText: "予約数" }),
      ).toBeVisible();
      await expect(
        page.locator("th").filter({ hasText: "記事数" }),
      ).toBeVisible();
      await expect(
        page.locator("th").filter({ hasText: "登録日" }),
      ).toBeVisible();
      await expect(
    ...
    test("should display role badges correctly", async ({ page }) => {
      ...
      const roleBadges = page
        .locator("td")
        .locator(
          '[class*="badge"], span:has-text("管理者"), span:has-text("ユーザー"), span:has-text("編集者"), span:has-text("閲覧者")',
        );
  ...
  test.describe("Create New User", () => {
    test("should display new user form", async ({ page }) => {
      ...
      await expect(
        page.locator('button:has-text("ロールを選択"), [data-value]'),
      ).toBeVisible();
    });
    ...
    test("should show validation errors for empty form", async ({ page }) => {
      ...
      await expect(
        page.getByText("パスワードは8文字以上必要です"),
      ).toBeVisible();
    });
    ...
    test("should create user with valid data", async ({ page }) => {
      ...
      const roleSelect = page
        .locator('button:has-text("ロールを選択"), [role="combobox"]')
        .first();
  ...
  test.describe("Delete User", () => {
    test("should show delete option in actions menu", async ({ page }) => {
      ...
      await expect(
        page.locator('[role="menuitem"]:has-text("削除")'),
      ).toBeVisible();
    });
    ...
    test("should show confirmation dialog before delete", async ({ page }) => {
      ...
      await expect(
        dialog.locator("text=この操作は取り消せません"),
      ).toBeVisible();
    });
    ...
    test("should show error when deleting user with related data", async ({
      page: _page,
    }) => {
    ...
    test("should prevent deleting own account", async ({ page }) => {
      ...
      await expect(
        page.getByText("自分自身を削除することはできません"),
  ...
  test.describe("Role Management", () => {
    test("should show role change option in actions menu", async ({ page }) => {
      ...
      await expect(
        page.locator(
          '[role="menuitem"]:has-text("管理者に変更"), [role="menuitem"]:has-text("ユーザーに変更")',
        ),
      ).toBeVisible();
    });

    test("should show confirmation dialog before role change", async ({
      page,
    }) => {
      await page.goto(urls.adminUsers);
      await waitForPageLoad(page);
      ...
      const roleChangeOption = page.locator(
        '[role="menuitem"]:has-text("管理者に変更"), [role="menuitem"]:has-text("ユーザーに変更")',
      );
    ...
    test("should display all role options in edit form", async ({ page }) => {
      ...
      await expect(
        page.locator('[role="option"]:has-text("スーパー管理者")'),
      ).toBeVisible();
      await expect(
        page.locator('[role="option"]:has-text("管理者")'),
      ).toBeVisible();
      await expect(
        page.locator('[role="option"]:has-text("編集者")'),
      ).toBeVisible();
      await expect(
        page.locator('[role="option"]:has-text("閲覧者")'),
      ).toBeVisible();
      await expect(
        page.locator('[role="option"]:has-text("ユーザー")'),
      ).toBeVisible();
    });
  });
  ...
  test.describe("Search and Filter", () => {
    ...
    test("should display role filter dropdown", async ({ page }) => {
      ...
      const roleFilter = page
        .locator('select[name="role"], [role="combobox"]:has-text("すべて")')
        .first();
    ...
    test("should filter users by role", async ({ page }) => {
      ...
      // Click role filter
      const roleFilter = page
        .locator('select[name="role"], [role="combobox"]')
        .first();
      await roleFilter.click();

      // Select ADMIN role
      await page
        .locator('[role="option"]:has-text("管理者"), option[value="ADMIN"]')
        .click();
  ...
  test.describe("Pagination", () => {
    test("should display pagination when there are many users", async ({
      page,
    }) => {
      await page.goto(urls.adminUsers);
      await waitForPageLoad(page);
      ...
      const pagination = page.locator(
        '[aria-label*="ページ"], nav:has(button:has-text("次")), button:has-text("次へ")',
      );
    ...
    test("should navigate to previous page", async ({ page }) => {
      ...
      const prevButton = page.locator(
        'button:has-text("前"), a:has-text("前")',
      );
  ...
  test.describe("Role-Based Access Control", () => {
    ...
    test.skip("should deny access to VIEWER role users for user management", async ({
      page,
    }) => {
      ...
      const deniedMessage = page.locator(
        "text=アクセス権限がありません, text=ユーザーが見つかりません",
      );
      await expect(deniedMessage.first()).toBeVisible();
    });

    test.skip("should deny access to EDITOR role users for user management", async ({
      page,
    }) => {
    ...
    test.skip("should allow ADMIN role users to manage users", async ({
      page,
    }) => {
  ...
  test.describe("Loading States and Error Handling", () => {
    ...
    test("should handle network errors gracefully", async ({
      ...
      const errorMessage = page.locator(
        '.bg-destructive, [role="alert"], text=エラー',
      );
