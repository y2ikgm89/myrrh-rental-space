...
test.describe('ブログ一覧ページ', () => {
  test('ブログ一覧ページが正しく表示される', async ({ page }) => {
    ...
    // 説明文を確認
    await expect(page.locator('text=ブログ記事の作成・編集・公開管理を行います')).toBeVisible()
  ...
  test('既存ブログ記事がテーブルに表示される', async ({ page }) => {
    ...
    if ((await table.count()) > 0) {
      ...
      await expect(page.locator('th').filter({ hasText: 'タイトル' })).toBeVisible()
      await expect(page.locator('th').filter({ hasText: 'カテゴリ' })).toBeVisible()
      await expect(page.locator('th').filter({ hasText: 'PV' })).toBeVisible()
      await expect(page.locator('th').filter({ hasText: '公開日時' })).toBeVisible()
  ...
  test('フィルター機能が動作する', async ({ page }) => {
    ...
    await expect(searchInput).toHaveAttribute('placeholder', 'タイトル、本文で検索...')
  })
...
test.describe('ブログ記事の新規作成', () => {
  ...
  test.skip('有効なデータで新規記事を作成できる（下書き）', async ({ page }) => {
    ...
    await page.fill('textarea[name="excerpt"], input[name="excerpt"]', '下書きテスト記事の抜粋です。')
    ...
    // 成功メッセージを待機
    await expect(page.locator('text=記事を作成しました')).toBeVisible({ timeout: 10000 })

    // 編集ページにリダイレクトされることを確認
    await page.waitForURL(/\/admin\/blog\/[a-f0-9-]+/, { timeout: 10000 })
  })

  test('新規作成ページでプレビューボタンをクリックすると通知が表示される', async ({ page }) => {
    await page.goto(urls.adminBlog + '/new')
    await page.waitForLoadState('networkidle')
    ...
    if ((await previewButton.count()) > 0) {
      ...
      await expect(page.locator('text=記事を作成後にプレビューできます')).toBeVisible()
    }
  })
})
...
test.describe('ブログ記事の編集', () => {
  test('編集ページが既存データで事前入力される', async ({ page }) => {
    ...
    // 最初の記事の編集ボタンをクリック
    const firstEditButton = page.locator('a[href*="/admin/blog/"]:has-text("編集")').first()
  ...
  test('記事情報を更新できる', async ({ page }) => {
    ...
    // 成功メッセージを確認
    await expect(page.locator('text=記事を保存しました')).toBeVisible({ timeout: 10000 })
  })
  ...
  test('未保存の変更がある場合に確認ダイアログが表示される', async ({ page }) => {
    await page.goto(urls.adminBlog)
    await page.waitForLoadState('networkidle')
...
test.describe('ブログ記事の削除', () => {
  test('削除ボタンをクリックすると確認ダイアログが表示される', async ({ page }) => {
    ...
    // ダイアログのタイトルを確認
    await expect(dialog.locator('text=ブログ記事を削除しますか？')).toBeVisible()
...
test.describe('公開状態の切り替え', () => {
  test('一覧ページのドロップダウンメニューで公開状態を切り替えられる', async ({ page }) => {
    await page.goto(urls.adminBlog)
    await page.waitForLoadState('networkidle')
  ...
  test('編集ページで公開ボタンが表示される（下書き記事の場合）', async ({ page }) => {
    // 下書き記事をフィルター
  ...
  test('編集ページで下書きに戻すボタンが表示される（公開記事の場合）', async ({ page }) => {
    // 公開記事をフィルター
...
test.describe('フォームバリデーション', () => {
  test('タイトルが空の場合にエラーが表示される', async ({ page }) => {
    ...
    // エラーメッセージを確認
    await expect(page.locator('text=タイトルは必須です')).toBeVisible({ timeout: 5000 })
  })
  ...
  test('スラッグが空の場合にエラーが表示される', async ({ page }) => {
    ...
    // エラーメッセージを確認
    await expect(page.locator('text=スラッグは必須です')).toBeVisible({ timeout: 5000 })
  })

  test('スラッグに不正な文字が含まれる場合にエラーが表示される', async ({ page }) => {
    await page.goto(urls.adminBlog + '/new')
    await page.waitForLoadState('networkidle')
    ...
    // エラーメッセージを確認
    await expect(page.locator('text=スラッグは小文字英数字とハイフンのみ')).toBeVisible({ timeout: 5000 })
  })
  ...
  test('抜粋が空の場合にエラーが表示される', async ({ page }) => {
    ...
    // エラーメッセージを確認
    await expect(page.locator('text=抜粋は必須です')).toBeVisible({ timeout: 5000 })
  })

  test('タイトルが200文字を超える場合にエラーが表示される', async ({ page }) => {
    await page.goto(urls.adminBlog + '/new')
    await page.waitForLoadState('networkidle')
    ...
    // エラーメッセージを確認
    await expect(page.locator('text=タイトルは200文字以内')).toBeVisible({ timeout: 5000 })
  })
})
...
test.describe('ページネーション', () => {
  test('ページネーションが表示される（記事が10件以上の場合）', async ({ page }) => {
    await page.goto(urls.adminBlog)
    await page.waitForLoadState('networkidle')

    // ページネーションコンポーネントを探す
    const pagination = page.locator('nav[aria-label*="ページ"], [class*="pagination"]')
  ...
  test('次のページに移動できる', async ({ page }) => {
    ...
    // 次へボタンを探す
    const nextButton = page.locator('button:has-text("次へ"), a:has-text("次へ"), button:has-text(">")')
  ...
  test('前のページに戻れる', async ({ page }) => {
    ...
    // 前へボタンを探す
    const prevButton = page.locator('button:has-text("前へ"), a:has-text("前へ"), button:has-text("<")')

    if ((await prevButton.count()) === 0 || (await prevButton.first().isDisabled())) {
...
test.describe('エラーハンドリング', () => {
  test('存在しない記事にアクセスすると404が表示される', async ({ page }) => {
    ...
    const notFoundText = page.locator('text=見つかりません, text=404, text=Not Found')
