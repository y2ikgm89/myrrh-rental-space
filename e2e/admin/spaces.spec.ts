...
test.describe('スペース一覧ページ', () => {
  ...
  test('既存スペースがテーブルに表示される', async ({ page }) => {
    ...
    // テーブルヘッダーを確認
    await expect(page.locator('th').filter({ hasText: 'スペース名' })).toBeVisible()
    await expect(page.locator('th').filter({ hasText: '住所' })).toBeVisible()
    await expect(page.locator('th').filter({ hasText: '定員' })).toBeVisible()
    await expect(page.locator('th').filter({ hasText: '時間料金' })).toBeVisible()
    await expect(page.locator('th').filter({ hasText: '公開状態' })).toBeVisible()
  ...
  test('フィルター機能が動作する', async ({ page }) => {
    ...
    // 公開状態フィルターが存在することを確認
    const publishFilter = page.locator('select[name="published"], [role="combobox"]').first()
...
test.describe('スペースの新規作成', () => {
  test('新規作成ページが正しく表示される', async ({ page }) => {
    ...
    await expect(page.locator('textarea, [contenteditable="true"]').first()).toBeVisible() // RichTextEditor
  ...
  test('有効なデータで新規スペースを作成できる', async ({ page }) => {
    ...
    await page.fill('input[name="capacity"]', testSpaces.roomA.capacity.toString())
    await page.fill('input[name="hourlyPrice"]', testSpaces.roomA.hourlyPrice.toString())
    ...
    if (await mainImageButton.count() > 0) {
      ...
      await page.fill('input[name="mainImageUrl"]', 'https://via.placeholder.com/400x300')
    }
    ...
    // 成功メッセージの確認（toast通知）
    const successToast = page.locator('text=スペースを作成しました, [role="status"]:has-text("スペースを作成しました")')
  ...
  test('設備を追加できる', async ({ page }) => {
    ...
    // 設備入力フィールド
    const facilityInput = page.locator('input[placeholder*="WiFi"], input[placeholder*="設備"]')
  ...
  test('追加画像を複数選択できる', async ({ page }) => {
    ...
    // メディアピッカー実装が必要
    test.skip(await addImagesButton.count() > 0, 'メディアピッカーの実装が必要')
  })
})
...
test.describe('スペースの編集', () => {
  test('編集ページが既存データで事前入力される', async ({ page }) => {
    ...
    // 最初のスペースの編集ボタンをクリック
    const firstEditButton = page.locator('a[href*="/admin/spaces/"][href*="/edit"]').first()
...
test.describe('スペースの削除', () => {
  test('詳細ページから削除できる', async ({ page }) => {
    ...
    // 最初のスペースの詳細ボタンをクリック
    const firstDetailButton = page.locator('a[href*="/admin/spaces/"]:has-text("詳細")').first()
    ...
    // 確認ボタンをクリック
    const confirmButton = confirmDialog.locator('button:has-text("削除"), button:has-text("確認")')
    ...
    // 成功メッセージの確認
    const successToast = page.locator('text=スペースを削除しました, [role="status"]:has-text("削除")')
    if (await successToast.count() > 0) {
      await expect(successToast.first()).toBeVisible()
    }
  })
...
test.describe('フォームバリデーション', () => {
  ...
  test('定員が1未満の場合にエラーが表示される', async ({ page }) => {
    ...
    // エラーメッセージを確認
    await expect(page.locator('text=定員は1以上で入力してください')).toBeVisible()
  })
  ...
  test('定員が1000を超える場合にエラーが表示される', async ({ page }) => {
    ...
    // エラーメッセージを確認
    await expect(page.locator('text=定員は1000以下で入力してください')).toBeVisible()
  })
  ...
  test('時間料金が負の数の場合にエラーが表示される', async ({ page }) => {
    ...
    // エラーメッセージを確認
    await expect(page.locator('text=時間料金は0以上で入力してください')).toBeVisible()
  })

  test('時間料金が1000000を超える場合にエラーが表示される', async ({ page }) => {
    await page.goto(urls.adminSpaces + '/new')
    ...
    // エラーメッセージを確認
    await expect(page.locator('text=時間料金は1000000以下で入力してください')).toBeVisible()
  })
  ...
  test('日額料金が負の数の場合にエラーが表示される', async ({ page }) => {
    ...
    // エラーメッセージを確認
    await expect(page.locator('text=日額料金は0以上で入力してください')).toBeVisible()
  })
})
...
test.describe('ページネーション', () => {
  test('ページネーションが表示される', async ({ page }) => {
    ...
    // ページネーションコンポーネントを探す
    const pagination = page.locator('[aria-label="ページネーション"], nav:has(button:has-text("次へ"))')
  ...
  test('次のページに移動できる', async ({ page }) => {
    ...
    const nextButton = page.locator('button:has-text("次へ"), a:has-text("次へ")')
