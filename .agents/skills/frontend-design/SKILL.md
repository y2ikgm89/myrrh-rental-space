...
1. `docs/reference/codex-rules/design-system-memory.md` を確認する
2. 補足設定が必要な場合は `docs/reference/codex-rules/project-design-config.md` を参照する

### 使用可能なアニメーションコンポーネント

`docs/reference/codex-rules/project-design-config.md` §モーション設計 に定義されたコンポーネントを使用。
一般的な構成:
...
`docs/reference/codex-rules/project-design-config.md` §ブランド のムードが確立済み。新セクション/ページもこの方向に従う。
大きく逸脱する場合はユーザーに確認。
...
```bash
python3 .agents/skills/ui-ux-pro-max/scripts/search.py "<keyword>" --domain style --stack nextjs
```
...
**Purpose**: [一文で目的]
**Mood**: (docs/reference/codex-rules/project-design-config.md のムード / 変更がある場合は明記)
...
### Typography
(docs/reference/codex-rules/project-design-config.md §タイポグラフィ から)

### Color Allocation
(docs/reference/codex-rules/project-design-config.md §カラーパレット から)
...
`docs/reference/codex-rules/project-design-config.md` §セクション設計 に準拠。
...
`docs/reference/codex-rules/anti-ai-design.md` のセルフレビュー 6 問を実施。3 問以上 yes → PASS。
...
| `docs/reference/codex-rules/anti-ai-design.md` | Anti-AI 強制ルール |
| `docs/reference/codex-rules/design-system-memory.md` | デザイン記憶プロトコル |
| `docs/reference/codex-rules/gsap-patterns.md` | GSAP / ScrollTrigger |
