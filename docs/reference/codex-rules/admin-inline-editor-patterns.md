...
旧 `docs/plans/059-unified-editor-sidepanel.md` / `071-unified-content-editor.md` は clean-break 原則（ADR-0015）により削除済み。過去の設計経緯は `git log --all --diff-filter=D -- docs/plans/059-* docs/plans/071-*` で辿る。現行の型名・ファイル名は **本ファイルとソース** を正とする。
