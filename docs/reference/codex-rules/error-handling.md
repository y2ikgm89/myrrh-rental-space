...
export const updateSpace = withPermission<[string, SpaceInput]>('space', 'update')(
  async (user, id, data) => {
...
```typescript
export const publishPost = withPermission<[string]>('post', 'publish')(
  async (user, id) => {
    ...
    await prisma.post.update({ where: { id }, data: { status: PostStatus.PUBLISHED } })
...
async function getPublishedPost(slug: string) {
  ...
  return await criticalFetch({
    fetch: () => prisma.post.findUnique({ where: { slug, status: PostStatus.PUBLISHED } }),
    category: ErrorCategory.DATABASE,
...
```typescript
import { ReservationOverlapError, isReservationOverlapError } from '@/shared/lib/errors'

try {
  await prisma.$transaction(async (tx) => {
    const overlap = await tx.reservation.findFirst({ /* 重複チェック */ })
  ...
  logError(error, { category: ErrorCategory.DATABASE, severity: ErrorSeverity.HIGH })
  return createFailure('予約の作成に失敗しました')
}
```
