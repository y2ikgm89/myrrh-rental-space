...
- 公開側で `useQueryState(s)` を使うため **`NuqsAdapter`** で `children` をラップする（`.claude/rules/nuqs-patterns.md`）。管理画面用アダプタ（`(dashboard)/layout.tsx`）とは Multiple Root Layouts により別 subtree
