...
- 管理画面での更新時に`revalidatePath('/blog')`で即座に再生成
...
**基本要件**:
- スラッグベースのURL（例: `/blog/space-usage-guide-2024`）
...
- 管理画面での更新時に`revalidatePath('/blog/[slug]')`で即座に再生成
...
**分類**:
- `categoryId`: String, 必須, FK → BlogCategories.id
...
- `GET /api/blog/posts`: ブログ記事一覧取得（公開API）
- `GET /api/blog/posts/[slug]`: ブログ記事詳細取得（公開API）
- `GET /api/blog/categories`: カテゴリ一覧取得（公開API）
- `GET /api/blog/tags`: タグ一覧取得（公開API）
