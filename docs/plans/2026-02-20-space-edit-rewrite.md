...
- [x] スペース一覧 → 編集ボタン → 編集ページが正常に開く
- [x] AdminDetailLayout のバックボタンが「詳細に戻る」で表示される
- [x] 管理サイドバーが消えないことを確認
- [x] スクロールバーが1本（ページ全体スクロール）になっていることを確認
- [x] フォーム編集後 Ctrl+S で保存できる
- [x] タブを閉じようとすると「変更が保存されていません」の警告が出る
- [x] 画像の D&D ソートが動作する
- [x] 設備の追加・削除が isDirty に反映される（保存ボタンが有効になる）
- [x] 新規作成ページが正常に動作する
- [x] SEO/OGP フィールドがメインフォームに表示される
- [x] DangerZone で削除確認ダイアログが表示される
- [x] 削除後 /admin/spaces にリダイレクトされる
