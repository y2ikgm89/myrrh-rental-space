...
| M-11                                                                                                | 公開 mutation の 4 段 guard pipeline gate が SSoT 配列 3 handler しか見ず、Turnstile を持つ公開 mutation の 3/18（17%）しか検査していない | **未対応**                   | 台帳 M-11。旧記載では M-11 が別内容に割り当てられており、この行は再評価から漏れていた                                                                                     |
...
| M-05 / M-07 / M-10 / M-32 / M-33 / M-34 / M-37                                                      | integration には guard がある（unit は無い）                                                                                              | 据え置き（緩和策を確認済み） | 下記「CI integration 実走の確認」および「integration guard の内訳」                                                                                                       |
| M-22 / M-27 / M-28（旧記載 M-24 / M-25 / M-26）                                                     | GCP 系（PAUSED / maxScale / traffic）の守り手不在                                                                                         | 据え置き継続                 | 実際に起きた欠陥ではない（round6 plan B の除外判断どおり）                                                                                                                |
| M-25 / M-26（旧記載 M-27 / M-28）                                                                   | `branch-protection.json` の paths filter / contexts 検査                                                                                  | 据え置き継続                 | live の branch protection は自動適用されず直接は弱まらない（同）                                                                                                          |
...
**M-05 の注**: integration テストが実行しているのは「P2002 を握りつぶす」方向だけで、
変異が壊した「P2002 以外は throw する」方向の見本が無い。
`refund-duplicate-detection.test.ts` は `isPrismaUniqueConstraintError` を実 DB のエラーで
検証するが、非 P2002 エラーを注入するケースを持たない。**この 1 件だけは integration でも掴めない。**
