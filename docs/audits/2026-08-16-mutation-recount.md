...
| M-11                                                                                                | 公開 mutation の 4 段 guard pipeline gate が SSoT 配列 3 handler しか見ず、Turnstile を持つ公開 mutation の 3/18（17%）しか検査していない | **対応済み（#2384）**         | 台帳 M-11。SSoT を実 handler 集合に広げ、新設 export が pipeline を外れる変異を検出する                                                                                   |
...
| M-22 / M-26                                                                                         | Cloud Scheduler PAUSED / branch-protection contexts の完全性                                                                              | **対応済み（#2388 / #2390）** | M-22: expected job は ENABLED 必須 + gcloud projection に state。M-26: contexts リテラルピン + ci.yml 除外除去                                                            |
| M-22 / M-27 / M-28（旧記載 M-24 / M-25 / M-26）                                                     | GCP 系（PAUSED / maxScale / traffic）の守り手不在                                                                                         | **対応済み（#2388 / #2392）** | M-22 ENABLED。M-27 maxScale=1。M-28 traffic latest 100%。いずれも live describe JSON を読む                                                                               |
...
**M-05 の注**: #2383 で unit 側に「P2002 以外は throw」方向をピンした。
