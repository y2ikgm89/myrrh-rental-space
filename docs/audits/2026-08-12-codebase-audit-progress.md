...
**高 11 件・中 64 件は全件クローズ。**残りは低 17 件で、[計画書 §6](../superpowers/plans/2026-08-13-codebase-audit-remediation.md#6-未着手の指摘台帳) の台帳に載っている。F-94 は R-03 の再掲として棄却へ移した（§2 には入れない）。
...
| [F-114](2026-08-12-codebase-audit-findings.md#f-114) | 低     | `fix/audit-f114-resend-multi-recipient`    | `to.length !== 1` の bounce / complaint / failed / suppressed では抑止しない。breadcrumb のみ残して 200 ack      | —                                                                                       |
