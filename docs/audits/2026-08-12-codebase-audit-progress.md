...
> **このファイルが状態の SSoT。未着手は無い。** 指摘の状態は**ここにしか書かない**。
...
> 計画の終了と残件の分類は [2026-08-15-codebase-audit-closeout.md](2026-08-15-codebase-audit-closeout.md)。

**読み方は 1 つだけ: ID がこの表に載っていれば済。** 載っていない確定指摘は無い（F-94 は棄却へ移した）。
...
**高 11 件・中 64 件・低 57 件は全件クローズ。**未着手の確定指摘は無い。F-94 は R-03 の再掲として棄却へ移した（§2 には入れない。findings 本文も外し R-62 だけを正とする）。webhook 境界の配線 mock（構造の穴 A）は本監査の §6 対象外。第 1–5 波で Stripe 決済書込の正本を実 DB に移し、重複する wiring mock 引数を外した。routing / orphan-claim の金額引数は呼出有無。イベント finalize の正本は実 DB。GCal / Resend / SwitchBot に決済書込は無い。cancelled orphan など Stripe API 依存の unit は残す。2026-08-15 品質 Wave で残件列の切り捨てを直すか製品判断で閉じた。残件が空でも「欠陥ゼロ」ではない。
...
| ID                                                   | 深刻度 | PR                            | 何をしたか                                                                                                           | 残件                                                                                    |
| ---------------------------------------------------- | ------ | ----------------------------- | -------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| [F-01](2026-08-12-codebase-audit-findings.md#f-01)   | 高     | #2214 / #2218                 | binary plan を artifact から除去。gate をパス包含判定に                                                              | —                                                                                       |
...
| [F-03](2026-08-12-codebase-audit-findings.md#f-03)   | 高     | #2227 → #2240 / #2242 / #2286 | `bodySizeLimit` を `MEDIA_MAX_SIZE_BYTES` 全体の最大値から導く。transport 失敗は `toast.error`                       | —                                                                                       |
| [F-04](2026-08-12-codebase-audit-findings.md#f-04)   | 高     | #2223                         | 繰返し予約の金額を instance ごとに解決                                                                               | —                                                                                       |
| [F-05](2026-08-12-codebase-audit-findings.md#f-05)   | 高     | #2245 / #2286                 | conform の `formatPaths` を使い、配列アイテムのエラーキーを揃える。`form.errors` を描画する                          | —                                                                                       |
...
| [F-10](2026-08-12-codebase-audit-findings.md#f-10)   | 高     | #2237                         | checkout の idempotency key を payload と一緒に動かす。品質 Wave で waitlist も `.../${expiresAt}` に揃えた          | —                                                                                       |
...
| [F-54](2026-08-12-codebase-audit-findings.md#f-54)   | 中     | #2256                         | Stripe 公式 Refund.status だけを通す。欠落・未知値は pending に落とさず、create は throw / webhook は persist しない | —                                                                                       |
...
| [F-65](2026-08-12-codebase-audit-findings.md#f-65)   | 中     | #2269                         | kill switch 用の短命キャッシュプロファイルを作り、反映上限を約 1 分に明示                                            | —                                                                                       |
...
| [F-63](2026-08-12-codebase-audit-findings.md#f-63)   | 中     | #2278                         | 必須セクションの複製を削除・表示切替と同じ述語で止める（UI も揃える）                                                | —                                                                                       |
...
| [F-71](2026-08-12-codebase-audit-findings.md#f-71)   | 中     | #2282                         | bot 判定をサーバー発行トークンへ。品質 Wave で欠落を fail-open しない                                                | —                                                                                       |
| [F-19](2026-08-12-codebase-audit-findings.md#f-19)   | 中     | #2286                         | 本番 seed は SEO / 組織 / 予約 singleton の update を空にする                                                        | —                                                                                       |
| [F-42](2026-08-12-codebase-audit-findings.md#f-42)   | 中     | #2286                         | 監査 CSV は期間必須・90 日・`take+1`。超過は 409。並びは `sequence asc`。品質 Wave で CSV リンクを期間必須にした     | —                                                                                       |
| [F-47](2026-08-12-codebase-audit-findings.md#f-47)   | 中     | #2286                         | チケット定員 floor をスロットごとの CONFIRMED 最大合計と比較する                                                     | —                                                                                       |
| [F-56](2026-08-12-codebase-audit-findings.md#f-56)   | 中     | #2286                         | 非整数アプリ金額は typed error。webhook は 2xx + CRITICAL + 管理者通知                                               | —                                                                                       |
| [F-69](2026-08-12-codebase-audit-findings.md#f-69)   | 中     | #2286                         | `getRequiredTermsByScope` を `criticalFetch` にし、失敗を未設定扱いしない                                            | —                                                                                       |
| [F-72](2026-08-12-codebase-audit-findings.md#f-72)   | 中     | #2286                         | `register()` は資格情報の同期検証だけ待つ。canary は `void` + `retry: false`                                         | 通常 purge の Retry-After は維持                                                        |
| [F-78](2026-08-12-codebase-audit-findings.md#f-78)   | 低     | #2286                         | webhook mock の `latestRefund` に `metadata` を戻し、ADMIN attribution を 1 本固定                                   | —                                                                                       |
| [F-80](2026-08-12-codebase-audit-findings.md#f-80)   | 低     | #2286                         | import 必須母集合に Cloudflare を入れ、inquiries bucket に `import {}` を足す                                        | —                                                                                       |
| [F-118](2026-08-12-codebase-audit-findings.md#f-118) | 低     | #2286                         | webhook 照合から `deletedAt` を外す。品質 Wave で PI 保存とイベント返金 find からも外した                            | —                                                                                       |
| [F-120](2026-08-12-codebase-audit-findings.md#f-120) | 低     | #2286                         | waitlist promote を行リースへ。品質 Wave で cron コメントの 728354 session lock 記述を直した                         | —                                                                                       |
...
| [F-98](2026-08-12-codebase-audit-findings.md#f-98)   | 低     | #2303                         | TabTitleNode / Lexical sanitize が type=`button`。品質 Wave で sanitizeRawEmbedHtml も type を固定                   | —                                                                                       |
...
| [F-104](2026-08-12-codebase-audit-findings.md#f-104) | 低     | #2306                         | 予約合計は `totalPriceWithTax` を `formatPrice`。ゲスト complete/cancel/status も同じ。再課税しない                  | —                                                                                       |
...
| [F-121](2026-08-12-codebase-audit-findings.md#f-121) | 低     | #2313                         | bulkMove のカテゴリ再確認を lock 直後へ。品質 Wave で trash-cleanup は生存 item 付きカテゴリを消さない               | —                                                                                       |
...
| #2229                 | 構造の穴 A の第一歩                                       | 自動返金コマンドを実 DB で走らせる層（予約側）。イベント webhook wrapper は #2286 |
| #2286                 | 構造の穴 B                                                | `cacheTag` producer と Cache-Tag ヘッダの突合 gate                                |
| #2286                 | 構造の穴 D                                                | events OFF 中の決済を、復帰後 expire が CANCELLED にしない                        |
| #2286                 | 構造の穴 F                                                | クーポン release を `releaseCouponUsage` に集約                                   |
| #2238 / #2239 / #2242 | 記録                                                      | 監査結果と修正計画を repo に入れ、記述を台帳と一致させる                          |

> **PR 欄の訂正履歴**: 16 行がブランチ名 `fix/audit-wave-1` を PR 欄に持っていた。このブランチは
> #2286 としてマージ済で ref は既に消えており、**追えない参照**だった（このファイルの存在理由は
> 「どの指摘をどの PR で塞いだか」を残すことなので、消えた ref は目的を果たさない）。`#2286` に統一した。
>
...
1. 新しい指摘は別監査として findings を増やす。この監査の未着手台帳は無い。
2. §1 の件数は §2 の行数から導く。**手で数えた値を書かない。**
   §2 の見出しにも件数を書かない — 書くと更新のたびに anchor が変わる。
3. 「残件」列に文があっても未着手ではない。採らなかった判断・範囲外・検証ギャップ・注記を書く。
   分類の正本は [2026-08-15-codebase-audit-closeout.md](2026-08-15-codebase-audit-closeout.md)。
   空欄にして済にしない — 次に触る人が「やらなかったこと」を見つけられるようにする。
4. 実装計画は消化しきったので削除した（[docs/README.md](../README.md) の lifecycle 規約）。
   **このファイルは残す** — 「どの指摘をどの PR で塞いだか」は git log から機械的には引けない。
   構造の穴 A–J の現在地は closeout §5。旧計画の実測文は git 履歴。
