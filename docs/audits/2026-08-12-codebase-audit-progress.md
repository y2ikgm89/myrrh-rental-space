...
**高 11 件・中 64 件・低 57 件は全件クローズ。**未着手は無い。F-94 は R-03 の再掲として棄却へ移した（§2 には入れない）。webhook 境界の配線 mock（構造の穴 A）は本監査の §6 対象外。第 1 波（`charge.refunded` attribution の実 DB 固定と配線 mock 引数テスト削除）は済。checkout fulfill / 他 webhook / 残りの payment unit は未着手。
