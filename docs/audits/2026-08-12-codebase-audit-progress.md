...
**高 11 件・中 64 件は全件クローズ。**残りは低 42 件で、[計画書 §6](../superpowers/plans/2026-08-13-codebase-audit-remediation.md#6-未着手の指摘台帳) の台帳に載っている。F-94 は R-03 の再掲として棄却へ移した（§2 には入れない）。
...
| [F-76](2026-08-12-codebase-audit-findings.md#f-76)   | 低     |                                            | `extractImportSpecifiers` が行/ブロックコメントを除去してから走査する。JSDoc 例示 import を辺にしない | —                                                                                       |
