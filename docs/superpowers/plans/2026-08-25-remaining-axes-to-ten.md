...
| 2026-08-26（夕） | 計画外の後追い。**P5 / P8 を解決**し、**P9 を部分解決**に更新。nightly と Lighthouse CI を廃止し広域 E2E を post-merge へ移した（#2691 / #2692）。水和前入力の取りこぼしを公開面・admin 面ともに塞ぎ（#2690 / #2693 / #2696）、タブは実リンク化した（#2700）。cron の一時停止検知（#2694）と 499 の取りこぼし（#2698）も入れた |
...
### 7.4 残っているもの（2026-08-26 時点、夕方に更新）

§6 に書いた「全部やっても残る」ものとは別に、**この計画の完了後に残っている作業**。

1. ~~`terraform apply` / Deploy Production が未実行~~ → **解決。** 2026-08-26 に 2 回
   デプロイした（`563c7645d` / `6f62bfb25`）。いずれも canary を `lastmod=15` で
   確認してから昇格。**ただし同日夕方に入れた monitoring 変更（#2694 / #2698）は
...
3. ~~nightly が現 HEAD で赤~~ → **nightly ごと廃止した。**
   ...
   Lighthouse CI は廃止した（#2691）。赤の原因だった
   `e2e/public/events-filters.spec.ts` の URL 同期は、水和前入力の取りこぼし
   （#2690 / #2693）とタブのリンク化（#2700）で潰してある。

   `--fail-on-flaky-tests` は**残す**。post-merge の赤は `post-merge-result` job が
   Issue 1 本で追跡し、次の緑で**自動的に閉じる**ので、一過性の flake は人の手を
   煩わせない。実例: 2026-08-26 10:49 に Issue #2699 が立ち、次の run で自動クローズ
   （内訳は 333 passed / 2 flaky / 0 failed）。
