...
| `prisma_pool_timeout`           | 5 超 / 5 min            | プール枯渇は公開面をまとめて 5xx にする。5 分継続でバジェットを急速に焼く。**満杯のプールで空きを待って諦めた分だけ**を数える（接続確立の期限切れとアイドル接続の切断は、プールの大きさと無関係なので含めない）。                                                                                                   |
| `cron_oidc_failure`             | 3 超 / 15 min           | 黙ってジョブが止まる事故用。401 / config 欠落専用で、cron ハンドラの 500 は拾わない。                                                                                                                                                                                                                               |
| `cron_job_failure`              | 3 超 / 15 min           | cron が 2xx を返さなかった回数の受け皿（監査 A-07）。5xx に加えて **499**（`attempt_deadline` と Cloud Run timeout がどちらも 300s で、期限切れの結果が 504 と 499 のどちらになるかは決まらない）。`retry_count = 3` を使い切った 1 tick が 4 件。endpoint 単位で group するので 1 本の停止が他 23 本に薄まらない。 |
