...
| `cron_heartbeat`                | absent over max silence | ジョブ未起動。2xx request log の `absent_over_time`。失敗 status（499/504/500）に依存しない。週次 2 本（`faq-stale-check` / `customer-risk-scan`）は user log-based PromQL の 25h 上限のため対象外。 |
