...
| `cron_heartbeat`                | absent over max silence | ジョブ未起動。2xx request log の `absent_over_time`。失敗 status（499/504/500）に依存しない。日次・週次は `condition_absent` 上限 23.5h を超えるので PromQL。 |
