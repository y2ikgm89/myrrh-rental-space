...
- Docker Compose 設定 `[feature:infra]`
  - docker-compose.yml 作成（PostgreSQL 16）
  - .env.local テンプレート作成
  - README に起動手順を追記
- 環境変数の分離
  - .env.local（開発: Docker PostgreSQL）
  - .env.production（本番: Supabase Cloud）→ 本番デプロイ時に作成
  - .env.example 作成

### 1.3 Prisma スキーマ設計 `DONE`

- データベース接続設定（Prisma 7 + pg adapter）
- Prisma スキーマ定義 `[feature:tdd]`
  - Users テーブル（Auth.js統合）
  - Spaces テーブル
  - Reservations テーブル
  - Inquiries テーブル
  - News テーブル
...
- Prisma Client 生成
- マイグレーション実行（Docker起動後） `cc:完了` (2026-01-07)
- シードデータ投入

### 1.4 認証システム実装 `DONE`

- Auth.js 5 設定（next-auth@beta）
- Prisma Adapter 設定
- JWT セッション管理
- 型定義拡張（next-auth.d.ts）
- proxy.ts（管理画面保護 - Next.js 16 推奨）
- API Route（/api/auth/[...nextauth]）
- RBAC 実装（Role: ADMIN/USER）
- ログインページ UI（フェーズ1.5で実装）
...
- フッターコンポーネント（DB 連携）
- 管理画面レイアウト
- 公開ページ用レイアウト（(public) route group）
- ログインページ UI
...
- パッケージインストール
  - 共通: `clsx`, `tailwind-merge`
  - 公開ページ用: `tailwind-variants`
  - shadcn/ui 依存: `@radix-ui/react-slot`, `class-variance-authority`
- `src/lib/utils.ts` 作成（cn 関数: clsx + tailwind-merge）

### 1.6.2 管理画面 UI 基盤（shadcn/ui + CVA） `cc:完了`

- ディレクトリ構造作成（`src/components/admin/`）
- shadcn/ui 初期化（出力先: `@/components/admin/ui`）
  - CVA は shadcn/ui に内蔵されるため追加インストール不要
- 基本コンポーネント追加（Button, Input, Card）
- globals.css に CSS 変数追加（テーマ用）
...
- ディレクトリ構造作成（`src/components/site/`）
- 公開ページ用カスタムコンポーネント（tv でバリアント定義）
  - `site/ui/Button.tsx`
  - `site/ui/Input.tsx`
  - `site/ui/Card.tsx`
  - `site/ui/Container.tsx`
- 既存レイアウトの移動
...
### 2.2 スペース詳細ページ `pm:確認済` (2026-01-08)
...
### 2.4 お問い合わせページ `pm:確認済` (2026-01-08)

- ✅ フォーム実装（ContactForm.tsx）
- ✅ バリデーション（Zod + Server Action）
- ✅ Server Action（submitContact）
- ✅ メール送信（Resend 統合）

### 2.5 お知らせページ `pm:確認済` (2026-01-08)

- ✅ 一覧表示（NewsList.tsx + nuqs ページネーション）
- ✅ 詳細表示（動的ルート `/news/[id]`）
...
- **2026-01-08**: フェーズ2 公開ページ実装レビュー承認済み（`cc:完了` → `pm:確認済`）
